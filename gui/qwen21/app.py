...
from gui.qwen21.strings import FIELDS_CN, LANGUAGES, UI
from gui.widgets import action_button, apply_variant
from library.env import resolve_under_home
from library.qwen21.requests import CacheRequest, TrainRequest, resolve_model_dir
...
_DIR_FIELDS = {"src", "out", "cache", "model_dir"}
_FILE_FIELDS = {"output"}
_PROGRESS_RE = re.compile(r"^\s+(step|text|latents) (\d+)/(\d+)")
...
def field_text(f: dataclasses.Field) -> tuple[str, str]:
    if _lang == "cn" and f.name in FIELDS_CN:
...
class RequestForm(QWidget):
    ...
    def _make(self, f: dataclasses.Field) -> tuple[QWidget, QWidget]:
        ...
        if f.name not in _DIR_FIELDS | _FILE_FIELDS:
            return w, w
        row = QWidget()
        h = QHBoxLayout(row)
...
class QwenWindow(DaemonJobMixin, QMainWindow):
    def __init__(self):
        ...
        self.tabs.addTab(
            self._page(
                self.train_form, self.train_summary, [self.btn_train, self.btn_chain]
            ),
            tr("tab_train"),
        )
    ...
    def _page(self, form: QWidget, summary: QLabel, buttons: list) -> QWidget:
        inner = QWidget()
        v = QVBoxLayout(inner)
        v.addWidget(form)
        v.addWidget(summary)
    ...
    def _set_busy(self, busy: bool) -> None:
        for b in (self.btn_cache, self.btn_train, self.btn_chain):
    ...
    def _reattach(self) -> None:
        ...
        if label not in (LABEL_CACHE, LABEL_TRAIN):
            return
        self._job_label = label
        self.status.setText(tr("running", label=label, job=job_id))
