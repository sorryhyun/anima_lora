...
STRINGS: dict[str, str] = {
    ...
    "models_btn": "モデル",
    "models_btn_tooltip": "Anima の重みをダウンロード / 再ダウンロード (ベース DiT、テキストエンコーダー、VAE、PE-Core、CJK 語彙パック)",
    "update_btn": "更新",
    ...
    "models_intro": "学習 / 推論の実行に必要な重みです。「初回セットをダウンロード」で Anima の重み 3 点、PE、タガーのチェックポイント、タグ DB を取得します。SAM3 や OCR スタックなどは「キュレーションモデル」にあります。ファイルは models/ に保存されます。",
    ...
    "model_vocab_pack": "CJK 語彙パック — 日本語 / 韓国語 / 中国語のキャプション行 (任意)",
    "model_pe_spatial": "PE-Spatial-B16-512 — REPA + 類似画像のグルーピング",
    "models_used_by": "使用箇所: {what}",
    "curation_models_btn": "キュレーションモデル",
    "curation_models_btn_tooltip": "anime_tools のキュレーション用重みをダウンロード (Anima タガー、SAM3、PE-Spatial、タグ DB、OCR)",
    "curation_models_title": "キュレーションモデル (anime_tools)",
    "curation_models_intro": "キュレーション各段階の重み — タグ付け、マスキング、グルーピング、キャプション修正、OCR。anime_tools のカタログを直接読みます。各段階は初回実行時に自分で取得もするため、ここでは待ち時間を移すだけです。",
