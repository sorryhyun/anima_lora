...
as the null model of our data term (\S\ref{sec:null}). SPD also
...
This section is definitions and derivations only; every demonstration
lives in \S\ref{sec:evidence}. It defines the estimand and the
resolution at which it can be known (\S\ref{sec:estimand}), decomposes
the perturbation demotion applies to a training gradient
(\S\ref{sec:branches}), derives the angular expansion of the gap and its
conditional reduction to a two-term account (\S\ref{sec:geometry}),
states the spectral argument as a null model of one branch
(\S\ref{sec:null}), and lists the discriminating predictions, each with
its designated probe (\S\ref{sec:predictions}).
...
For an arm $a \in \{\mathrm{nat}, \reenc, e\}$, let
...
$\Delta_e(\sigma) = d_e(\sigma) - d_{\reenc}(\sigma)$, where the
re-encoding control (decode--re-encode at native resolution) prices the
VAE round trip that demotion necessarily pays; $\Delta_e$ can be
...
\paragraph{The aggregation operator is part of the estimand.}
$d_e$ can be aggregated over a probe set per-example (cosine per image,
then mean --- the batch-1 worst case) or batch-aggregate (gradients
summed over the batch before the cosine --- the object SGD actually
follows). These are different estimands, and no coefficient fitted on
one is guaranteed to transfer to the other; the application section
publishes one map for each (\S\ref{sec:map}).

\paragraph{Resolution: what an instrument can certify.}
Any estimate of Eq.~\ref{eq:gapdef} compares finite-draw estimates of
population directions, so every claim carries the instrument's
resolution. For a route whose true excess is zero, the paired estimate
$\bar\Delta$ fluctuates with standard error
$\mathrm{SE}(N, D, \sigma\text{-bin})$, so the one-sided 95\% upper
bound concentrates near $1.645\,\mathrm{SE}$: no margin smaller than the
\emph{median certification resolution}
\begin{equation}
\epsstar(N, D, \text{bin}) \;=\; 1.645\,\mathrm{SE}(N, D, \text{bin})
\label{eq:epsstar}
\end{equation}
is certifiable even for a truly safe route.\footnote{\emph{Median}
because at margin exactly $1.645\,\mathrm{SE}$ a truly-safe route
certifies with only ${\sim}50\%$ power; the general margin at size
$\alpha$ and power $1{-}\beta$ is
$\epsstar_{\alpha,\beta} = b_D + (z_{1-\alpha} + z_{1-\beta})\,
\mathrm{SE}$, with $b_D$ a residual finite-draw bias allowance.
Eq.~\ref{eq:epsstar} is the $\beta = 0.5$, $b_D{=}0$ slice.} We
\emph{define} ``safe at margin $\varepsilon$'' as the non-inferiority
statement
\begin{equation}
...
\label{eq:safe}
\end{equation}
Two consequences hold before any measurement. A ``safe'' verdict is
always a statistical non-inferiority claim at instrument resolution ---
never a hard bound; a true gap below $\epsstar$ is undetectable by
construction. And $\epsstar$ is the honest currency for negative claims
too: ``no certifiable window'' means the route's upper bounds never
cross below $\varepsilon$ at the current $(N, D)$, which a larger
campaign could change --- whereas ``certifiably unsafe'' (lower bound
above $\varepsilon$) cannot.

\subsection{Data and graph perturbations}
\label{sec:branches}

Demotion changes exactly two things about the map from (image, noise,
caption) to adapter gradient. Writing the adapter gradient as
$g = J^{\!\top} r$ (input--output Jacobian times prediction residual),
the perturbation $\delta g = \bar g_e - \bar g_{\mathrm{nat}}$
decomposes as
\begin{equation}
\delta g \;=\;
...
\paragraph{The graph branch has identifiable parts.}
...
\subsection{Angular geometry: the four-term expansion and its
reduction}
\label{sec:geometry}

Cosine geometry fixes how a perturbation is read. With
$G(\sigma) = \|\bar g_{\mathrm{nat}}\|$ and $P_{\!\perp}$ the projector
orthogonal to $\bar g_{\mathrm{nat}}$, the local identity
$1 - \cos(g, g + \delta g) \approx
\tfrac{1}{2}\|P_{\!\perp}\delta g\|^2 / G^2$ holds in the small-mismatch
limit. Substituting Eq.~\ref{eq:branches} and expanding the square gives
the \emph{four-term angular expansion} --- derived unconditionally from
the perturbation expansion plus the local cosine identity:
\begin{equation}
d_e(\sigma) \;\approx\;
...
\label{eq:fourterm}
\end{equation}
where $R'_e$ collects the remainder branch and the beyond-quadratic
cosine terms. $S_e$ and $F_e$ are non-negative; $I_e$ can carry either
sign --- a fact that matters in \S\ref{sec:confronted}.
...
it is not licensed far from the small-mismatch regime.

\paragraph{The exact angular form.}
For a perturbation that is purely orthogonal at relative size
$\kappa = \|\delta g_\perp\| / G$, the cosine is exactly
$1/\sqrt{1 + \kappa^2}$, so the corresponding exact gap is
\begin{equation}
d \;=\; 1 - \frac{1}{\sqrt{1 + \kappa^2}}\,,
\qquad \kappa_e(\sigma) = c_e \cdot m(\sigma)/G(\sigma),
\label{eq:exact}
\end{equation}
which saturates instead of overshooting when $\kappa$ is comparable to
one, and whose small-$\kappa$ limit is Eq.~\ref{eq:twoterm}'s data term
with $A_e = c_e^2/2$. The linear mismatch loading
$\|\delta g_\perp\| = c_e\, m(\sigma)$ is an \emph{empirical}
ingredient, not a derivation --- so wherever a fitted functional form is
reported, Eq.~\ref{eq:exact} is labeled the \emph{best empirical
predictor} and Eq.~\ref{eq:twoterm} the \emph{derived small-perturbation
form}; \S\ref{sec:confronted} scores both, held out.

\paragraph{The ledger.}
Explicitly: the four-term expansion (Eq.~\ref{eq:fourterm}) is derived
unconditionally; the two-term reduction (Eq.~\ref{eq:twoterm}) is
derived \emph{conditional on A1--A4}, each of which is tested by a
designated probe (\S\ref{sec:predictions}); the coefficients $A_e$ (or
$c_e$), $m(\sigma)$, $G(\sigma)$, $\Floor_e$ are measured, not derived;
and any fitted power or loading is labeled empirical. The account was
...
\subsection{Discriminating predictions and falsifiers}
...
names the designated probe that discharges it in \S\ref{sec:evidence};
several double as falsifiers of the account's own assumptions.
...
The evidence discharges the theory one term at a time: the instrument
...
\paragraph{Finite draws manufacture floors.}
The naive estimate of Eq.~\ref{eq:gapdef} --- redraw floor minus a
...
signature as ``absolute token count sets the floor.'' A variance
back-of-envelope predicts spurious endpoint gaps of
${\approx}\,0.02 / 0.05 / 0.15$ for $896/768/512$ --- plausibly
${\sim}40\%$ of the uncorrected intermediate floors --- and a
retroactive scan confirms the confound is live: the $1024{\to}896$
endpoint gap decays as ${+}0.100 / {+}0.035 / {-}0.016$ at
$D = 4/8/16$, a clean $c/D$ decay with fitted $c \approx 0.5$. No
...
uncorrected endpoint self-cosine reads $0.79$--$0.85$, but a nested draw
sweep ($D = 4 \ldots 64$) extrapolates it to $1.005$ (bootstrap 95\% CI
...
($|c| \le 0.05$ for the $512$ route, against $c \approx +0.29$
uncorrected) --- the attenuation correction removes exactly the $1/D$
component the sweep isolates.
\end{enumerate}
...
--- is designed to measure this dependence rather than concede it, and
its adapter training is in progress at the time of writing
\textbf{[pending]}.
...
\paragraph{Debiasing (\S\ref{sec:instrument}), implementation.} Under
