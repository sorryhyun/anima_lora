...
\begin{abstract}
Training a diffusion transformer consumes compute superlinearly in
resolution, and a recent family of methods therefore trains or samples
at reduced resolution on a spectral premise: at high noise, a
downscaled latent carries almost the full surviving signal. But
low-resolution steps are reported to deliver a training signal that
differs from native --- these methods observe a training--inference
gap and retrain against stage-specific targets rather than
substituting latents under the native objective. We measure
directly \emph{when} a downscaled training step yields the native
gradient, with a debiased gap estimator (naive finite-draw estimation
manufactures the very effect under test). The substitution's cost
splits in two: a noise-dependent term governed by the downscale
...
itself. No noise level rescues a $2\times$ downscale; but on the
mildest route ($1024{\to}896$) the training signal is indistinguishable
from native at instrument resolution for $\sigma > 0.5$ --- a window no
spectral tolerance predicts, and which our two-term account predicts on
held-out routes at $0.07$--$0.09$ RMSE. Gating substitution on that
window gives \emph{selective} low-resolution training: $-14.6\%$
wall-clock at a fixed step budget, with a sample-level footprint within
the training-seed lottery (quality non-inferiority at production scale
remains open).
\end{abstract}
...
distribution, only the spatial grid changed on some steps?

Under flow-matching noising
...
By this spectral premise, substitution is safe \emph{whenever the noise
...
$\sigma \approx 0.5$ --- $3.5\times$ the equal-power prediction ---
while a $2\times$ downscale is certifiably unsafe at \emph{every} noise
level, retaining a gap of ${\approx}\,0.30$ on pure-noise input
(\S\ref{sec:instrument}, Fig.~\ref{fig:verdict}). And no choice of
tolerance rescues the spectral family: the $\delta$ tuned to the one
safe boundary predicts the failing routes safe, the published default
predicts every route safe, and the zero-free-parameter tolerance our
own pipeline fixes --- the measured noise floor of the resize--encode
round trip itself --- moves all three boundaries together to
$\sigma \approx 0.98$ while still admitting no floor
(\S\ref{sec:phenomenon}). The family is ratio-only and floorless; the
measurement is neither. Nor does the premise's original evidence
settle the question: it lives at the level of generated output, and a
small quality-score difference does not transfer to a small difference
in what a training step \emph{learns}.
...
measured resolution --- and the correction cut both ways, halving one
route's raw floor while surfacing a small real endpoint gap that
single-estimate variance had buried on another.
...
amplitude while absolute size governs the floor; the null is scored
across its entire tolerance family, including a parameter-free anchor
measured from the pipeline's own re-encode noise floor, and no member
reproduces the map; the account \emph{predicts} held-out routes at
${\sim}0.07$--$0.09$ RMSE, with the floor law hitting two floors it
never saw. The exact angular form of the account resolves a systematic
overshoot its quadratic reduction leaves (\S\ref{sec:confronted}).
...
$\sigma$-conditional trainer that substitutes the downscaled latent
only on measured-safe steps. On a fixed-step exercise grid ($5$ arms
$\times$ $3$ seeds $\times$ $2$ corpora, bit-exact paired draws) it
realizes $-15.1\%$ forward FLOPs and $-14.6\%$ wall-clock while
perturbing endpoint samples no more than a change of training seed
(the seed-noise yardstick, Table~\ref{tab:yardstick});
...
Each claim is tagged with its epistemic status (pre-registered /
confirmatory / post-hoc) where it is discharged, rather than hedged
...
whose per-band phases accumulate with grid coordinates --- the
grid-dependent coordinate system whose contribution to the demotion
floor \S\ref{sec:decomposition} isolates by intervention. Fine-tuning is
...
inference-time resolution schedule follows.
...
with routes it never saw (\S\ref{sec:confronted}, Fig.~\ref{fig:e5}).
...
\label{fig:estimator}
\end{subfigure}
\caption{The verdict, before the apparatus: (a)~safety is a per-route
boundary --- $\sigma \approx 0.5$ on the mildest route, no certifiable
window for $768$, unsafe at every noise level for $512$ --- against
the spectral family's prediction of $\sigma \approx 0.14$--$0.20$
(Fig.~\ref{fig:spectrum}, \S\ref{sec:phenomenon});
(b)~the estimator context those curves are read through.}
\label{fig:verdict}
\end{figure}
...
\S\ref{sec:confronted}), under three functional forms for the data term
(shared floor structure): the fitted-power form, the derived
small-perturbation quadratic (Eq.~\ref{eq:twoterm}), and the exact
angular link (Eq.~\ref{eq:exact}). Left: the held-out route
$1024{\to}768$ --- every curve is predicted from governors fitted on
\emph{other} routes; its mid-$\sigma$ dip below the predicted floor is
form-independent (\S\ref{sec:confronted}). Middle/right: the fit routes
$1024{\to}896$ and $1024{\to}512$. The second held-out route,
$1280{\to}1024$ --- where the exact link's saturation removes the
quadratic's peak overshoot --- is Appendix Fig.~\ref{fig:e51280}.}
\label{fig:e5}
\end{figure}
...
$\sigma \approx 0.5$, the $768$ route never certifies, and the
...
through: the redraw-floor cosine --- what finite draws do to the
estimator --- and the U-shaped gradient norm $G(\sigma)$ that shapes
...
$1024$-tier natives demoted to $896/768/512$ --- the numbers
Fig.~\ref{fig:gapcurves} plots (the uncorrected historical
...
no member of the null family can produce. There is no noise level at which a
...
\subsection{The spectral null, confronted at the boundary level}
...
\label{fig:sigmaeq}
\end{subfigure}
\caption{The spectral account, computed from the measured RAPSD:
safety predicted from $\sigma \approx 0.14$. Against the measurement
...
$\sigma$ at any tolerance. No row reproduces the measured pattern of
...
(Table~\ref{tab:null}). The parameter-free null is therefore maximally
...
member of the family --- it predicts the $2\times$ downscale becomes
safe, when no noise level does. Either way the confrontation does not
hinge on the choice: the structural failures (N2, N3) are
$\delta$-independent.
...
measured re-encode noise floor --- the zero-free-parameter anchor our
own pipeline fixes. No row reproduces the
...
instead it \emph{establishes} the shape factorization directly. The decay is
...
monotone maximum in every arm of both runs --- the predicted $m/G$
shape, discharged. This is a post-hoc consistency analysis (so labeled): it
...
and with it the null's only degree of freedom. (The $896{\to}768$ read
...
plateau --- ratio $0.857$ against the passing $0.875$ --- while the
null puts these two boundaries within $0.01$ of each other at every
...
account on three routes, then predict two routes it never saw, from
governors alone (Fig.~\ref{fig:e5}). Fit routes: $\{1024{\to}896,\ 1024{\to}512,\ 1280{\to}1120\}$;
...
equivalently, graph-relative stationarity fails there: the reduced
...
demote--re-promote probe --- which isolates the data
...
Against the account's discriminating predictions, the
...
corrected formula but a measured map at a stated resolution --- and the
...
the pre-debiasing instrument --- conservative reads, since the
...
sample-level reads (\S\ref{sec:trainer}) --- at $480$ steps, not
