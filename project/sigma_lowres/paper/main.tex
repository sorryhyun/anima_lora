...
the null model of the data term's input-mediated part, a null that is
...
\paragraph{Demotion.}
A \emph{demotion} $e_{\mathrm{src}} {\to} e$ replaces the native latent
with a coarser-grid one, produced exactly as a scale-wise trainer would
produce it; $e_{\mathrm{src}}$ denotes the route's source (native) edge
and $e$ its target edge. With $y$ the source image, $\mathcal{R}_e$ the pixel-space
downscale to the tier-$e$ bucket (native aspect ratio preserved), and
$\mathcal{E}$ the VAE encoder,
\begin{equation*}
x_{\mathrm{src}} \;=\;
\mathcal{E}\big(\mathcal{R}_{e_{\mathrm{src}}}(y)\big)
...
Three \emph{arms} recur throughout: $\mathrm{src}$, the source arm,
which trains on the native-resolution latent $x_{\mathrm{src}}$; $e$,
the target arm, which trains on the demoted latent $x_e$; and $\reenc$,
...
\begin{equation}
d_e(\sigma) \;=\; 1 - \cos\!\big(\bar g_{\mathrm{src}}(\sigma),\,
\bar g_{e}(\sigma)\big)
\;=\; \tfrac{1}{2}\,\big\|\hat g_{\mathrm{src}} - \hat g_{e}\big\|^2,
\label{eq:gapdef}
\end{equation}
with $\hat g$ the unit-normalized gradients; $d_e \ge 0$ always. The
subscript on $d_e$ names the route's target edge, the source grid being
fixed by context (tables write the full route $e_{\mathrm{src}} {\to} e$
where several sources appear). The
...
$\delta g = \bar g_e - \bar g_{\mathrm{src}}$ decomposes as
\begin{equation}
\delta g \;=\;
...
One qualification is worth stating precisely: $J_{\mathrm{src}}$ and
...
$G(\sigma) = \|\bar g_{\mathrm{src}}\|$ and $P_{\!\perp}$ the projector
orthogonal to $\bar g_{\mathrm{src}}$, split the perturbation into a
rescaling and a rotation:
$\kappa_\parallel = \hat g_{\mathrm{src}}^{\!\top}\delta g \,/\, G$
and $\kappa_\perp = \|P_{\!\perp}\delta g\| \,/\, G$. Substituting
$\bar g_e = \bar g_{\mathrm{src}} + \delta g$ into
...
quadratic order; hence $m(\sigma)$ in Eq.~\ref{eq:twoterm} is read net
...
The spectral argument reviewed in \S\ref{sec:related} was advanced as
a license for inference-time substitution; no prior work applies it to
the training gradient, so the null model below is our own transport of
the argument to the training question, not a claim its authors make.
Made precise, the argument is a model of one part of one branch of what
demotion perturbs, and its strongest available form is SPD's
...
band's activation time, with no graph share and no interaction:
\begin{equation}
...
Nyquist sits at $f_{\mathrm{cut}} = \tfrac{1}{2}\,e/e_{\mathrm{src}}$,
...
that no member of the null family can produce. There is no noise level at which a
...
shape. This is a post-hoc consistency analysis (so labeled): it
...
$i$ at $i \cdot H_{\mathrm{src}}/H_{\mathrm{dem}}$ per axis), making its
...
than the null on its committed region ($\sigma > \sigma_{\mathrm{eq}}$,
where the null predicts gap $0$: RMSE $0.164$ null vs $0.096$ ours);
...
Bayes-residual mismatch, measured $G$, route gain calibrated on the one
safe route),
