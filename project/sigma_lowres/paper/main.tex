...
model and objective, has not been measured; existing methods sidestep
the question by retraining against stage-specific targets. We measure
...
tokens --- a window no spectral tolerance predicts, and which our
...
spatial frequencies drop below the noise floor first --- so above a
...
matching \citep{jin2024pyramidal}, progressive-resolution training
more broadly
...
--- whether a downscaled step delivers the native gradient under an
unchanged objective --- and that is the different route this paper
takes. What a training step consumes is the expected adapter gradient,
and demotion perturbs more than the network's input: the regression
...
one-sided non-inferiority statement, never an absolute claim. From the
...
universal mismatch shape read through the U-shaped gradient norm, plus
a $\sigma$-independent graph floor. The account connects directly to
the inference-side literature: the spectral argument, in its strongest
tolerance-parameterized form (SPD, \citealp{xiao2026spd}), enters as
the null model of the data term's input-mediated part --- ratio-only,
and floorless by construction.
...
never saw --- a tool for anticipating a route's gap before measuring
it. Gating substitution on the measured-safe windows gives an opt-in
...
gap, states the spectral argument as a null model of its data term, and
constructs the debiased estimator; \S\ref{sec:evidence} builds the
instrument and discharges the account's predictions against the null;
...
masks. Where the premise does enter training, as in pyramidal flow
...
and objective fixed --- and we find input-level closeness does not
...
\label{eq:safe}
\end{equation}
never a hard bound. Throughout, ``no certifiable window'' means a
route's upper bounds never cross below $\varepsilon$ at the current
$(N, D)$, while ``certifiably unsafe'' means its lower bound stays
above it.
...
the expansion reduces to the \emph{two-term account}, stated on the
reported form of the estimand, the re-encoding excess of
\S\ref{sec:estimand}:
\begin{equation}
\gap_e(\sigma) \;:=\; \Delta_e(\sigma) \;=\; d_e(\sigma) -
d_{\reenc}(\sigma)
\;\approx\;
...
reduction survives the subtraction of the control because the
$\reenc$ arm pays no grid change: it contributes no graph share, and
its data share is precisely the re-encode component of $B_e$
(Eq.~\ref{eq:branches}), which the excess cancels to the expansion's
quadratic order --- so $m(\sigma)$ in Eq.~\ref{eq:twoterm} is read net
of re-encode, and the four-term structure of $d_e$ carries over to
$\Delta_e$ term by term. The
