...
\begin{equation}
d_e(\sigma) \;=\; 1 - \cos\!\big(\bar g_{\src}(\sigma),\,
\bar g_{\dem}(\sigma)\big)
\;=\; \tfrac{1}{2}\,\big\|\hat g_{\src} - \hat g_{\dem}\big\|^2,
\label{eq:gapdef}
\end{equation}
with $\hat g$ the unit-normalized gradients; $d_e \ge 0$ always. One
...
the subscript on $d_e$ --- and on every route-level scalar to follow
--- is the route's \emph{target edge}, an edge value, never an arm
label, so it instantiates numerically ($\Floor_{1120}$,
$\mathrm{Resid}_{768}$), with the source edge fixed by context (tables
write the full route $e_0 {\to} e$ where several sources appear). The
...
(\S\ref{sec:estimand}), and the control's own expansion is degenerate:
the $\reenc$ arm changes no grid, so $\Delta J = 0$ --- no graph
share, no interaction --- and its data branch is the re-encode
perturbation alone, collapsing Eq.~\ref{eq:fourterm} to
$d_{\reenc} \approx S_{\reenc} + R'_{\reenc}$, with $S_{\reenc}$ the
re-encode share (measured ${\approx}\,0$ where probed: the control row
of Table~\ref{tab:floor}). Subtracting the two expansions,
\begin{equation*}
\Delta_e(\sigma) \;\approx\;
...
\emph{input-mediated part of the data branch}. It does not
constrain the target-mediated share, the graph branch, the
...
Nyquist sits at $f_{\mathrm{cut}} = \tfrac{1}{2}\,e/e_0$,
...
through the account's} $\|\bar g_{\src}\|$ \emph{and a calibrated route gain}, not a
...
\mid z)$ \citep{xing2026divunc}, set to zero by the diagonal null ---
