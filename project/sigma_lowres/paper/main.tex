...
has masked the frequencies the coarse grid loses}.

Whether that premise licenses a training-side substitution is a
different question. What a training step consumes is the expected
adapter gradient, and demotion perturbs more than the network's input:
the regression target carries the clean image at unit weight at every
noise level, and the compute graph itself changes with the grid. Nor
does the premise's original evidence settle the matter: it lives at the
level of generated output, and a small quality-score difference does
not transfer to a small difference in what a training step
\emph{learns}. This paper measures the question directly, at the
...
The paper proceeds theory-first. \S\ref{sec:related} situates the
question; \S\ref{sec:theory} defines the estimand and its certification
resolution, derives the angular account of the demotion gap, states the
spectral argument as a null model of its data term, and constructs the
debiased estimator; \S\ref{sec:evidence} builds the instrument and
discharges the account's predictions against the null;
\S\ref{sec:practical} gives the resulting safety map and the
$\sigma$-conditional trainer. Each claim is tagged with its epistemic
status (pre-registered / confirmatory / post-hoc) where it is
discharged.
...
$\sigma \approx 0.5$, $768$ has no certifiable window, and $512$ never
enters at any noise level --- against the spectral family's prediction
...
redraw-floor cosine and the U-shaped gradient norm $G(\sigma)$
(\S\ref{sec:anatomy}).}
\label{fig:verdict}
\end{figure}
...
$B{=}1000$; $\bar m$ and $G$ are run-level instruments, held fixed).
...
estimate lands dead-on, but its own sampling spread is wide: a
...
(Fig.~\ref{fig:e5}) --- structural, not sampling noise: a positive
...
yardstick's lesson --- point reads ride the draw --- applies equally to
...
structural, not sampling luck --- while the floor law's headline hit
softens from ``dead-on'' to ``consistent'': $F(2160)$ bootstraps to
...
\emph{measured} map (\S\ref{sec:map}), never the predicted one, for
exactly this reason.
