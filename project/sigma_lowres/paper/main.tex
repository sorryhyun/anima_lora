...
spectral account governed, the answer would be yes above a per-route
boundary --- for our latents, $\sigma \approx 0.14$ at the classical
equal-power tolerance, and $\sigma \approx 0.6$ even at the strict
tolerance SPD ships (\S\ref{sec:null}). The payoff would be large: on
our corpus roughly half of all training draws sit at $\sigma>0.5$.
...
$g = J^{\!\top} r$ (Jacobian times residual), demotion perturbs exactly
two things --- the information the input still carries, and the compute
graph itself --- and first-order perturbation plus cosine geometry force
the form
$\gap_e(\sigma) \approx A_e\, m(\sigma)/G(\sigma)^p + \Floor_e$
with $p \in [1,2]$: a noise-dependent input branch read through the
U-shaped gradient norm, plus a $\sigma$-independent floor carried by the
network function. The derivation fixes the \emph{form}; the coefficients
are measured, and we keep an explicit ledger of what is derived, what is
assumed, and what is measured. We then embed the spectral argument, in
its strongest tolerance-parameterized form (SPD, \citealp{xiao2026spd}),
as the null model of the input branch --- and show, before any gradient
is measured, that on our latent spectrum the null is a one-parameter
family that depends on the downscale \emph{ratio} alone, is compressed to
a ${\sim}0.13$ spread in $\sigma$ across all tolerances, and structurally
admits no floor.
\item \textbf{Measurements that reject the null on every axis where the
accounts diverge} (\S\ref{sec:experiments}). Our instrument computes, per
noise-level bin, the cosine between adapter gradients from native and
downscaled latents, calibrated by a fresh-noise floor and a VAE
re-encoding control. The measured crossover for the mildest downscale
($0.875\times$ edge) is $\sigma\!\approx\!0.5$; no tolerance reproduces
the map (the one matching this route predicts the two never-safe routes
safe by $\sigma = 0.63$); a $2\times$ downscale never becomes safe,
retaining a gap of ${\sim}0.3$ at $\sigma=0.94$ where the input is
essentially pure noise. Term by term: the floor is real,
$\sigma$-independent, governed by absolute target token count rather than
ratio, concentrated in early blocks, and decomposes --- via an exact
positional-interpolation intervention on the rotary embedding --- into a
coordinate-system share $\mathrm{RoPE}_e$ and a non-positional graph
residue $\mathrm{Resid}_e$; the input branch's amplitude is governed by
the downscale ratio (an iso-severity probe at $1.6\times$ different
absolute size reproduces the same curve); and the prediction-residual
mismatch is \emph{route-uniform}, placing all route identity in the
Jacobian --- the residual-level kill of the diagonal spectral null.
...
finding that resolution changes are not free at the training level. Our
contribution is orthogonal to all of these: we measure whether input-level
...
the cross term second-order. One caveat is worth stating precisely:
...
residual level: the posterior mean is a per-band Wiener shrinkage, so with
...
legitimately come from. SPD's shipped $\delta = 0.01$ is an
...
the tolerance $\delta$ sweeps its range. SPD ships $\delta = 0.01$; the
...
$0.01$ & $0.61$ & $0.63$ & $0.72$ & SPD's shipped default \\
...
\caption{Latent RAPSD and the equal-power slice
$\sigma_{\mathrm{eq}}(f)$ of the spectral family. Predicted $\sigmastar$:
$0.136$ / $0.146$ / ${\sim}0.20$ for demotion to 896 / 768 / 512.}
\label{fig:rapsd}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.495\textwidth}
...
\label{fig:gapcurves}
\end{subfigure}
\caption{The spectral family (left, equal-power slice) versus the gradient
measurement (right). No tolerance reproduces the measured pattern
...
over almost the entire $\sigma$ axis (Fig.~\ref{fig:rapsd}). The full
...
$0.62$, while SPD's own shipped $\delta = 0.01$ predicts all three routes
safe by $0.72$;
