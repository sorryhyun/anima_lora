...
$u^{\perp} = u - (\hat g^{\!\top}u)\,\hat g$ for the component of a
vector orthogonal to $g$ (the main text's notation); decompose
...
\paragraph{The drop bounds (Eq.~\ref{eq:dropbounds}).} Both bounds are
Cauchy--Schwarz on the expansion above. For the interaction,
\begin{equation*}
|I_e| \;=\; \frac{\big|\langle B_e^{\perp},\,
C_e^{\perp}\rangle\big|}{G^2}
\;\le\; \frac{\|B_e^{\perp}\|\,\|C_e^{\perp}\|}{G^2}
\;=\; 2\sqrt{S_e\,F_e}\,.
\end{equation*}
For the remainder, write $\rho_e = \|R_e^{\perp}\|^2/(2G^2)$ and note
$\|(B_e{+}C_e)^{\perp}\|^2/(2G^2) = S_e + F_e + I_e$; the
$R_e$-bearing terms of the expansion then obey
\begin{equation*}
\frac{\big|2\,\langle (B_e{+}C_e)^{\perp},\, R_e^{\perp}\rangle
\;+\; \|R_e^{\perp}\|^2\big|}{2G^2}
\;\le\; 2\sqrt{\big(S_e + F_e + I_e\big)\,\rho_e} \;+\; \rho_e\,,
\end{equation*}
with the beyond-quadratic terms of Eq.~\ref{eq:appquad} supplying the
$O(\kappa_\perp^2\kappa_\parallel,\, \kappa_\perp^4)$ tail.
...
$\delta g$ is read into gap units. What \emph{is} derivable from first
principles is only a one-sided envelope,
...
route-specific directions of \S\ref{sec:anatomy} license no such
axiom. Hence the loading stays an assumption with a designated probe
...
functional forms for the data term of \S\ref{sec:confronted}
...
amplitude $m(\sigma)$ from the residual probe, and each run's own bin-mean
gradient norm $\|\bar g_{\src}(\sigma)\|$ (gap and norm always from the same process, per the
...
itself (the noise ceiling for a 3-dof curve); the spectral baseline is
gap $= 0$ on the spectral account's committed region
...
the bin-mean $\|\bar g_{\src}(\sigma)\|$ are run-level instruments with no per-image
decomposition and are held fixed. The replica includes the committed
floor law's positive-floor filter ($\Floor > 0.005$): in $41\%$ of
draws the resampled $1120$ floor clears the filter and the law
re-anchors on the $(896, 1120)$ legs rather than $(512, 896)$,
branch variability the bands therefore contain. The $896$ curve in
Fig.~\ref{fig:e5} is the post-hoc leave-$896$-out lane under the exact
link ($c$ from the ratio-twin $1120$ alone $= 0.142$ vs $0.150$ fitted
on $896$ itself; floor law through $512{+}1120$, whose fragile
$\Floor_{1120}$ leg enters the log fit clamped positive ($36\%$ of
bootstrap draws clamp), giving $F(3012) = +0.012$ vs measured
$+0.019$ $[+0.010, +0.030]$; curve RMSE $0.073$), the X-form
counterpart of the quadratic-form addendum
(\S\ref{sec:confronted}).

\paragraph{Interventional $B/C$ ledger (\S\ref{sec:confronted}),
implementation.} Routes $1024{\to}\{896, 768, 512\}$,
$\sigma \in [0.5, 1.0]$ in four bins plus the $\sigma{=}1$ endpoint
bin, $D{=}8$ draws per bin, $N{=}24$ images, deterministic mode, two
independent draw sets per arm. Each arm's draw-summed adapter
gradient is stored per bin; the interventional split is
$B = \bar g_{\mathrm{repromote}} - \bar g_{\src}$ (the data
intervention at fixed native graph: the demote--re-promote arm's
input passed through the downscale$\to$upscale$\to$encode pipeline
but is evaluated on the native grid) and
$C = \bar g_{\mathrm{demote}} - \bar g_{\mathrm{repromote}}$ (the
graph intervention at fixed demoted data). The quadratic shares $S$,
$F$, $I$, and $\rho = \cos(B^{\perp}, C^{\perp})$ use cross-draw-set
inner products to cancel shared draw noise; the debiased estimates
are not confined to $[-1, 1]$ (hence $\rho = -1.24$ at one
low-amplitude bin). The exact counterfactual angles $h(B)$, $h(C)$,
$h(B{+}C)$, with
$h(X) = 1 - \cos(\bar g_{\src}, \bar g_{\src} + X)$, are computed on
the raw arm means. The re-encoding proxy check re-references the data
leg against the control arm,
$B_{\reenc} = \bar g_{\mathrm{repromote}} - \bar g_{\reenc}$, and
compares $|B^{\perp}_{\reenc}|$ to $|B^{\perp}|$ per bin.

\begin{table}[t]
\caption{The interventional $B/C$ ledger behind
\S\ref{sec:confronted}: per (route, $\sigma$-bin) orthogonal
...
\paragraph{Spectral-account computation.} Table~\ref{tab:null} evaluates
Eq.~\ref{eq:spd} at the demoted grids' Nyquist frequencies
$f_{\mathrm{cut}} = \tfrac{1}{2}\,e/e_0$ on the $N{=}40$
...
(one-signed bias; paired same-grid contrasts), not cited as debiased
evidence.
...
(\S\ref{sec:anatomy}) shows the underlying mismatch directions are
image- and route-specific.}
\label{tab:residual}
\centering
\small
\begin{tabular}{lccccc}
