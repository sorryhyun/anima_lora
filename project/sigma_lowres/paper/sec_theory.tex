...
Assumptions (i) and (ii) are not unstructured hopes: both discarded
terms obey exact bounds in the retained shares
(Cauchy--Schwarz on each inner product; Appendix~\ref{app:geometry}),
\begin{equation}
...
remainder branch's own share. The first bound says the interaction can
never exceed the retained sum ($2\sqrt{S_eF_e} \le S_e + F_e$) and is
automatically negligible wherever one share dominates the other, so
(ii) carries independent content only near share crossover. The second
says the first-order part of the remainder enters at half power ---
one factor of $\sqrt{\rho_e}$ per retained share --- so (i) reduces to
the branch-level statement that $R_e$ is small against $B_e$ and
$C_e$, plus the small-$\kappa$ domain for the tail. The control
counterpart $R'_{\reenc}$ needs no bound of its own: the control's
degenerate expansion $d_{\reenc} \approx S_{\reenc} + R'_{\reenc}$ is
itself the measured near-zero row of Table~\ref{tab:floor}, which pins
$R'_{\reenc}$ jointly with $S_{\reenc}$.
Applied in that order,
\begin{align}
\gap_e(\sigma)
\;&\approx\; \big(S_e - S_{\reenc}\big) \;+\; F_e
&&\text{by (i), (ii) via Eq.~\ref{eq:dropbounds}}
\nonumber\\
\;&\approx\;
