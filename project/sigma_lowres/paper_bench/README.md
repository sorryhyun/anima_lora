...
| [E25d](experiments/e25d/) | truthful low-res inference labeling (res-cond intended-use transfer) | **REGISTERED (FROZEN) 2026-08-12** | when the rescond model renders at the trained 896 sibling grid, does the truthful label s=−0.193 move output toward the same ckpt's own 1024 render vs the false label s=0? CRN pair (same grid+seed ⇒ same init noise), gate = paired PE-cos Δ vs ref1024, median per ckpt, TRANSFER/ANTI/NULL at ≥5/6; 6 rescond ckpts × SFW paper prompts × 3 arms = 189 renders one boot; constant s only (σ-gated schedule = recorded follow-up needing own registration); does NOT pre-read or refine E25c. ≈ 1–1.5 GPU-h. `e25d_render_read.py` |
