...
| [E25b](experiments/e25b/) | explicit resolution conditioning (adaln micro-conditioning arm) | **STAGE 1 IMPROVES 2026-08-12; STAGE 2 FROZEN 2026-08-12** | 25b-1 **IMPROVES** (median r_h 0.572 ≤ 0.9, median Δρ +0.005; 5/5 bins readable both stores, all gates pass; shallow-σ concentrated, graph leg h(C) shrinks at every bin); **Stage 2 RUN + READ same day: 25b-2 FAIL (quality)** — rescond below the in-batch yardstick on both corpora (hews 0.9450 vs 0.9578, channel 0.9515 vs 0.9541; near-miss = miss), throughput free (paired wall 1.0024), ΔW secondary opposite-direction 12/12 (0.41-0.47 vs combo 0.74-0.77), **25b-3 NULL** (hews +0.006 / channel −0.021) + not rescued; verdict story: conditioning absorbs the substitution per step but converges to a *different* model — no ship, no paper-method update, flag stays experimental; gradient-level positive + render-level negative both to paper-2. σ-draw-shift observation recorded (37.5% vs 50.8% σ>0.5 mass at same seeds vs 8/02; cross-batch scalars dead again). Read: `e25b_ship_read.{py,json}`. **Post-close 2026-08-12**: trained lever measured **~90 % common-mode** — ‖W·φ(0)‖ 0.130–0.192 vs resolution-differential 0.005–0.024, cos ≥ 0.994, all 12 carriers (`e25b_native_offset.{py,json}`, the registered-but-unemitted Stage-2 descriptive) → the divergence channel is the parameterization's common-mode freedom, motivates E25e |
| [E25c](experiments/e25c/) | res-cond projection as an inference-time resolution knob | **FROZEN 2026-08-12** | mod-guidance-class knob (`--res_cond_s`, needs a rescond ckpt; NOT Spectrum-class); one render sweep S={−0.415,−0.193,0,+0.193,+0.415} on the Stage-2 rescond ckpts, E4 prompt grid; 25c-1 axis-vs-lookup (median 5-point Spearman of high-band RAPSD vs s, ±0.8) gating 25c-2 gap-shrink (paired Δcos vs native twin, ≥5/6 pairs); 3-point-axis extrapolation honesty recorded; zero training, ≈ 1.5–2.5 GPU-h; blocked on the Stage-2 grid |
| [E25d](experiments/e25d/) | truthful low-res inference labeling (res-cond intended-use transfer) | **DONE 2026-08-12 — 25d-1 ANTI** | truthful label s=−0.193 at the trained 896 sibling grid moves renders *away* from the ckpt's own 1024 behavior on 5/6 ckpts (paired PE-cos Δ vs ref1024, CRN pairs) — but signal is **hews-borne** (3/3 negative, 5/27 prompts positive) while channel is a prompt-level coin flip (18/36; alone it would read NULL); effects small (only hews_s1002 −0.0167 exceeds 0.01) vs a 0.07–0.12 baseline gap the label closes none of; high-band RAPSD shows no spectral compensation (34/63 cells). Reading: label = weak style perturbation, not a grid compensator — the 25b-1 per-step absorption does NOT transfer to full renders (consistent with 25b's divergent-endpoint story). Intended-use knob **closed at this operating point**; reopening is training-side (E25-family amendment). `--res_cond_s` stays experimental opt-in; E25c untouched. Read: `e25d_render_read.py`, `e25d_result.json` |
| [E25e](experiments/e25e/) | re-centered resolution conditioning (common-mode channel removed) | **RUN + READ 2026-08-12: 25e-1 COLLAPSE-PERSISTS, 25e-2 PASS** | `--sigma_lowres_res_cond_centered`: delta = W·(φ(s) − φ(0)) — native (s=0) forwards bit-identical to control for ANY projection + zero grad to it (structurally demoted-only; the ~90 % common-mode channel E25b measured cannot exist); stamp `ss_sigma_lowres_res_cond="centered"`, `TestResCondCentered` invariants. One paper-equivalent grid, no pre-gate (user decision): 2 corpora × 3 seeds × (native/combo/rescond_c) + determinism dup = 19 runs, renders + in-batch yardstick one boot; **25e-1** mechanism read (in-batch ΔW cos to native twin: COMMON-MODE-CONFIRMED / COLLAPSE-PERSISTS / NULL at −0.05/−0.15 judgment constants) + **25e-2** ship read (25b-2 quality bar verbatim; PASS = paper-method update, no product ship); rows independent (CONFIRMED+FAIL legal and informative). ≈ 4–4.5 GPU-h. **RUN + READ same day, all 4 gates pass**: **25e-1 COLLAPSE-PERSISTS** — median ΔW cos rescond_c 0.440/0.515 vs combo 0.744/0.770 (Δ −0.303/−0.254, both far past −0.15): removing the common-mode channel did NOT recover the endpoint; the divergence is the demote-step co-adaptation itself, conditioning-lever family closes for ship purposes, the ~90 % common-mode measurement demoted to descriptive. **25e-2 PASS** — rescond_c inside the in-batch yardstick on both corpora (hews 0.9587 vs 0.9571, channel 0.9654 vs 0.9541; combo itself outside on hews at 0.9546, seed lottery), throughput free (paired wall 0.9997). The COLLAPSE+PASS branch as registered: renders fine off a divergent endpoint — ΔW closeness ≠ render closeness a third time, opposite direction. Learned differential norms 0.003–0.077, strongly seed-variable. Read: `e25e_read.{py,json}` |
