...
## E4 — the end-to-end A/B [DONE 2026-07-30 — exercise grid + 5th arm; residuals retired]

*Residual retirement (2026-07-30): the full-band CMMD rescoring and
val-loss/peak-mem additions formerly tracked in
`required_experiments.md` are closed out — the 5-arm yardstick +
narrowed §5 claim ("not yet run" limited to the CMMD gate) stand as the
shipped read, and the paper does not gate on a full-band CMMD verdict.
If the σ-gate-vs-896only tension (below) is ever re-litigated, that
rescoring is the instrument to revive.*
...
## E7 — controlled-LoRA 2×2 style factorial [DONE 2026-07-29/30 — moved verbatim from required_experiments.md]
