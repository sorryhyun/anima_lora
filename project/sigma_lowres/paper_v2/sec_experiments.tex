...
decomposition of Eq.~\ref{eq:branches} measures when both legs
are resolved as vectors. The instrument is the probe of
\S\ref{sec:instrument} run with three additional arms, realizing the
repromote construction of \S\ref{sec:ouraccount} directly:
$B = \bar g_{\rp} - \bar g_{\reenc}$ (the data leg, re-referenced
against the control so that the VAE round trip is netted out; the two
referencings agree to ${\sim}4\%$ wherever the leg carries signal,
Appendix~\ref{app:instrument}) and
$C = \bar g_{\dem} - \bar g_{\rp}$ per (route, $\sigma$-bin),
...
$\rho = \cos(B^{\perp}, C^{\perp}) \in [-0.96, -0.63]$, pooling to
$\bar\rho = -0.91$ over the gated verdict bins. The crossing geometry of
