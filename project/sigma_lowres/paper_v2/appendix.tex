...
$\reenc$, $\rp$, $\dem$) mark \emph{whose} gradient, latent, or
Jacobian an object is; the subscript on every route-level scalar is the
...
$\sigma \ge 0.875$, the $1/B$ term dominating
there. What the collapse does not bound is the intercept, the
coherent drift no batch size removes. A debiased verdict grid with
pooled self-floors (pool $4$, $N{=}40$; paired arm$-$reenc gaps at
$B \in \{1, 4, 40\}$) bounds it: the $a + b/B$ fit's intercept is
${\leq}\,0.02$ ($1024{\to}896$) and ${\leq}\,0.03$ ($1024{\to}768$)
at $\sigma \geq 0.44$, but persists over the lower bins
($+0.04$--$0.09$, $+0.05$--$0.28$, and $+0.15$--$0.60$ for
$896/768/512$) — aggregation confers safety only where the per-example
map of \S\ref{sec:map} is already near-safe. The small real $896$
endpoint gap ($+0.042$ in this run's $B{=}1$ read) averages out in the
aggregate ($+0.001$).
...
\S\ref{sec:estimand}. The three arms of Eq.~\ref{eq:branches} ---
$\src$ (native content, fine graph), $\rp$ (demoted content, fine
graph), $\dem$ (demoted content, coarse graph) --- are values of one
map on the data--graph lattice, and their adapter gradients all live
in the shared adapter-parameter space, so the differences are literal
and the split
\begin{equation}
\delta g \;=\; \bar g_{\dem} - \bar g_{\src}
\;=\;
\underbrace{\big(\bar g_{\rp} - \bar g_{\src}\big)}_{\text{data
branch }B_e}
\;+\;
\underbrace{\big(\bar g_{\dem} - \bar g_{\rp}\big)}_{\text{graph
branch }C_e}
\label{eq:appbranches}
\end{equation}
telescopes exactly: Eq.~\ref{eq:branches} is an identity at every
amplitude, with no remainder and no linearization. The lattice's
fourth corner, native content on the coarse graph, does not exist ---
representing native content on a coarser grid \emph{is} demotion ---
so the path through $\rp$ is the only realizable one and the split is
unique (Fig.~\ref{fig:ledgergeom}).

What each leg collects follows from the mean-factor expansion. Split
each factor into its noise mean and fluctuation,
$J_a = \bar J_a + \tilde J_a$ and $r_a = \bar r_a + \tilde r_a$ with
...
\label{eq:appmean}
\end{equation}
Applying Eq.~\ref{eq:appmean} to the two arms of each leg, with the
demoted data's mean factors written as source plus a perturbation
($\bar r_{\dem} = \bar r + \Delta\bar r$,
$\bar J_{\dem} = \bar J + \Delta J$; bare bars denote the source
arm): the data
branch collects everything the data intervention does at fixed
graph --- through the residual (its leading term is the pulled-back
residual perturbation $\bar J^{\!\top}\Delta\bar r$), through
the input-content dependence of the Jacobian at fixed grid, and
through the shift of the noise-covariance coupling
$\mathbb{E}_\epsilon[\tilde J_a^{\!\top}\tilde r_a]$. The graph
branch collects everything the grid change does at fixed data: its
leading term is the changed pull-back on the residual,
$\Delta J^{\!\top}\bar r$, and because the graph difference is taken
at the \emph{perturbed} data, the second-order coupling
$\Delta J^{\!\top}\Delta\bar r$ rides inside it, along with the
grid-driven covariance shift. Relative to a direct two-factor product
rule between $\src$ and $\dem$, the terms that expansion would have
collected as a remainder --- the second-order coupling and the
covariance differences --- appear \emph{inside} the legs, assigned to
whichever intervention realizes them along the path; nothing is
discarded.

One remark delimits what is and is not computable. When two arms
share a grid (as for the re-encoding control, or the $\src$--$\rp$
pair) every operation above is literal. Across grids, $J_{\src}$ and
$J_{\dem}$ act on different token counts, so the mean-factor
differences $\Delta J$ and $\Delta \bar r$ are matrix formulas only
under a fixed identification of the two output spaces (e.g.\ spectral
...
leaving the legs untouched. This is why the paper treats the
mean-factor forms as the legs' leading-order \emph{interpretation}
and measures only the legs themselves, via the designated probes of
\S\ref{sec:ourscored}; no cross-grid matrix product is ever
computed.

\paragraph{The mean-factor reading of the near-cancellation.}
Read through Eq.~\ref{eq:appmean}, the measured anti-alignment of
\S\ref{sec:observations} has a natural factor-level account. The two
legs move the two factors separately: the data leg holds the graph
fixed and shifts the residual factor (its leading term the fine
pull-back $\bar J^{\!\top}\Delta\bar r$, plus the input dependence
of the Jacobian at fixed grid), while the graph leg holds the data
fixed and shifts the pull-back ($\Delta J^{\!\top}$ acting on the
demoted-content residual). The factors are coupled through what they
are evaluated at (\S\ref{sec:estimand}): the Jacobian moves with the
data, the residual with the graph. Moved \emph{together} --- the
substitution's own endpoints, native content on the fine graph and
band-limited content on the grid whose Nyquist matches it --- the
two factor changes compensate inside the product to the extent that
the base computation is consistent across its resolution tiers,
which is the property the demotion recipe banks on and whose
gradient-level expression is the smallness of the measured excess.
Moved \emph{separately} --- the waypoint, band-limited content on
the fine graph --- no compensation is available: the fine graph
responds to content that no longer fills it, and that mismatch
response is created by the data leg and removed by the graph leg. To
leading order the same vector therefore enters the two legs with
opposite sign --- the factor-level restatement of the round-trip
geometry of Appendix~\ref{app:instrument}
(Fig.~\ref{fig:ledgergeom}) --- and what survives to the realized
gap is the part of the pull-back change that co-moves with the grid
rather than the content: the grid-anchored positional phase and
token-count statistics collected by the floor. The measured reads of
\S\ref{sec:observations} are consistent with this reading at every
point where they can check it: the anti-alignment is a pull-back
property (weak at the residual level, deep in gradient space), it is
uniform wherever branch energy is, and aligning the rotary phases to
native geometry removes part of the graph leg's ability to mirror
the data leg (halved $\rho$, doubled $h(C)$). The reading remains an
interpretation, not a derivation: the factor forms are cross-grid
objects only under a fixed identification (the remark above), the
mixed term $\Delta J^{\!\top}\Delta\bar r$ is not small at the
measured amplitudes, and the closure results of
\S\ref{sec:observations} bar re-deriving any ledger term at the
estimand level.
...
two-term reduction consumes the measured curve as-is
...
This appendix derives the two angular forms displayed in
...
therefore exactly the saturating form displayed as Eq.~\ref{eq:exact}
in \S\ref{sec:reduction}. Since
...
$\delta g = B_e + C_e$ (Eq.~\ref{eq:appbranches}) into the leading
...
\end{equation*}
exactly --- the legs telescope, so no cross term with a remainder
branch arises. Divided by $2G^2$, the three terms are the data share
$S_e$, the graph share $\Phi_e$, and the projected interaction $I_e$
of the four-term expansion, Eq.~\ref{eq:fourterm}. The remainder
$R'_e$ is therefore purely geometric, with an exact expansion: the
beyond-quadratic terms of Eq.~\ref{eq:appquad} and the
parallel-component correction, each suppressed by an extra power of
perturbation size relative to the retained terms in the
small-$\kappa$ domain.
...
its graph leg is identically zero, and both control terms built from
it vanish: its graph share $\Phi_{\reenc}$ and its projected
interaction $I_{\reenc}$ each carry a factor of the vanished
leg. Writing
...
\paragraph{The interaction bound, and the remainder's domain.} The
retained interaction obeys an exact Cauchy--Schwarz bound in the
retained shares, and the remainder is a pure power of the
...
\label{eq:dropbounds}
\end{equation}
The first says the interaction can never
exceed the retained sum ($2\sqrt{S_e\Phi_e} \le S_e + \Phi_e$) and
would be automatically negligible wherever one share dominates the
other; measured, the interference saturates this bound nearly
everywhere (the near-cancellation of \S\ref{sec:observations}), which
is why the main text carries it as entry (ii)'s measurement rather
than an assumption. The second restates that the remainder is pure
expansion geometry: it carries no branch-level content to bound, and
its size is governed by the perturbation amplitude alone. Entry (i)
of \S\ref{sec:reduction} handles it by domain rather than by
assumption --- at the measured in-window amplitudes
($\kappa_\perp \approx 0.5$--$1.0$) the truncation is out of domain,
which is exactly the regime where the unit rule of
\S\ref{sec:reduction} forbids quadratic magnitude reads and routes
every magnitude through the exact link, under which nothing is
truncated (the recorded ${\sim}3$--$4\times$ underprediction of
$h(B{+}C)$ by the truncated ledger, below, is this term made
visible). The
...
\S\ref{sec:instrument}. The redraw floor plays the same pricing role
for finite-draw noise.
...
$D \le 32$ sets; prefix sums, no extra forwards) and the $c/D$ fit of
\S\ref{sec:instrument} is run on route means with a bootstrap CI
over images. Finite-draw cosines are kernel-path sensitive: twin runs
...
extrapolates it to the unit self-cosine reported in
...
($2.6 \to 0.4$ at $\sigma\!\approx\!0.3 \to 9.3$;
...
$\epsilon$. A monotone absolute mismatch over this U-shaped norm
yields the measured mid-$\sigma$-peaked curves: bin-mean gap
tracks $1/\|\bar g_{\src}\|$ (Spearman $+0.55$--$+0.90$). The
renormalization check reads the amplification
...
which is the low-norm amplification behind the mid-$\sigma$ peak of the
measured curves. This shape read is post-hoc consistency analysis;
all safety criteria stay in cosine units, since direction is what a
...
(\S\ref{sec:ourscored}), a forward-only comparison of the model's
caption-conditioned prior across grids, subsequently dissociated the
prior from the floor's route ordering: its route distances are flat
where the floors are strongly ordered, ruling out a
resolution-conditioned prior or a training-distribution discontinuity
as the carrier. In the target-strength ($\alpha$) sweep the
mid-sweep points $\alpha \in [0.5, 0.75]$ are unreadable by construction
...
the low-norm amplification above, surfacing along the $\alpha$ axis.
The well-conditioned anchors behind the $\alpha$-flatness claim of
\S\ref{sec:ourscored}: $768$ reads $+0.070 \pm 0.015$ at $\alpha{=}0$
vs $+0.049 \pm 0.010$ at $\alpha{=}1$ (control slope $+0.003$), and
$512$ reads $+0.269 \pm 0.041$ vs $+0.337 \pm 0.067$.
...
$896$ and $768$ the floors sit at or below the verdict grid's
resolution $\epsstar$, which the endpoint sweeps resolve only
...
floor is what the plateau converges to once the data term vanishes and
the estimator can resolve it.
...
why the exact link is the form displayed in Fig.~\ref{fig:accounts}.}
\label{fig:e51280}
\end{figure}
...
$+0.075$. Every form falls within the union of the measured CI and the
...
realizes the legs of Eq.~\ref{eq:branches}:
$B = \bar g_{\mathrm{repromote}} - \bar g_{\src}$ (the data
...
confined to $[-1, 1]$ (hence $\rho = -1.24$ at one low-amplitude
bin).
...
exact by construction. One caveat on the units of
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/ledger_geometry.png}
\caption{\textbf{The ledger's geometry.} Left: the three arms as
corners of the data--graph lattice. $B$ and $C$ are the partial
differences along the lattice's only realizable path — the fourth
corner (native content on the coarse graph) does not exist — so the
split is a round trip off the matched configuration
($\bar g_{\src}$, $\bar g_{\dem}$) through the single mismatched
corner ($\bar g_{\mathrm{repromote}}$). Right: the measured legs
tip-to-tail at $\sigma = 0.563$ (Table~\ref{tab:e9ledger}), to a
common scale in units of $\|\bar g_{\src}\|$; the drawn angle is the
debiased $\rho$. Amplitude-matched legs leave only a small resultant
($896$, $768$); the $512$ graph leg is ${\sim}1.8\times$ the data
leg, and the unmatched share survives as the realized gap.}
\label{fig:ledgergeom}
\end{figure}

\paragraph{The ledger's geometry: why the legs anti-align.}
The near anti-parallelism of Table~\ref{tab:e9ledger} is structural
rather than incidental (Fig.~\ref{fig:ledgergeom}). The three arm means are values of one map on a
$2 \times 2$ data--graph lattice — $\bar g_{\src}$ at (native
content, fine graph), $\bar g_{\mathrm{repromote}}$ at (demoted
content, fine graph), $\bar g_{\dem}$ at (demoted content, coarse
graph) — all living in the shared adapter-parameter space, so the
differences are literal, and $B$ and $C$ are the partial differences
along the path (native, fine) $\to$ (demoted, fine) $\to$ (demoted,
coarse). That is the lattice's only realizable path: its fourth
corner, native content on the coarse graph, does not exist, since
placing native content on the coarse grid \emph{is} the demotion
pipeline — the coarse graph forces the demoted data. The endpoints
are the two content--graph \emph{matched} corners (native content on
its own grid; band-limited content on the grid whose Nyquist matches
it); the unavoidable waypoint is the single mismatched corner. The
split is therefore a round trip off the matched configuration and
back — $B$ creates the content--graph mismatch at fixed graph, $C$
removes it at fixed data — so, to leading order in the mismatch,
whatever share of each leg passes through it is the same vector with
opposite sign, which fixes the sign of $\rho$. Its depth is then
bookkeeping: projection off $\hat g_{\src}$ is linear, so
$|(B{+}C)^{\perp}|^2 = |B^{\perp}|^2 + |C^{\perp}|^2
+ 2\rho\,|B^{\perp}||C^{\perp}|$, and the ledger carries two
independent measurements — how far the waypoint sits from the corners
(the leg amplitudes) and how nearly the endpoints coincide (the
resultant) — with $\rho$ their law-of-cosines consequence; the
separately debiased sign column is a consistency check on the raw
$h(\cdot)$ reads, not a third fact. Two corollaries follow, and the
data confirms both. Route verdicts reduce to amplitude matching, the
realized $h(B{+}C)$ collapsing where
$|B^{\perp}| \approx |C^{\perp}|$ (the window-center localization of
Appendix~\ref{sec:headtohead}); and what no matching can cancel is
the graph-intrinsic share of $C$ with no data-side counterpart — the
resultant collects exactly the residue the floor ledger below
decomposes into $\mathrm{RoPE}_e + \mathrm{Resid}_e$, and at the
$\sigma = 1$ endpoint, where the input is $\epsilon$ on every arm and
the data leg has nothing left to act on, that uncancelled residue is
...
$\sigma$-resolved follow-up forecloses the route this suggests
...
test is predictive: fit the two-term reduction on three routes, then
...
All four pre-registered gates pass (Table~\ref{tab:headtohead},
...
$896$ floor at $+0.019$ under the fitted-power form ($+0.012$ under
the exact link, Appendix~\ref{app:instrument}) against the measured
$+0.019$ $[+0.010, +0.030]$. The floor law is thus correct on both floors
outside its fit set, though its point estimates vary with the draw: a
...
held-out accuracy comes from precisely the two ingredients it adds.
...
Fig.~\ref{fig:e51280}); the fitted governors themselves come out
form-invariant. Two things
the fit does \emph{not} establish: the prediction is not within
the verdict resolution anywhere ($\chi^2$/bin $7.6$--$9.2$ held-out
...
functional form is unidentified at our operating points
(the identifiability analysis is in Appendix~\ref{app:instrument}).
...
(Fig.~\ref{fig:accounts}): a positive two-term reduction cannot produce a gap
...
rather than entry (ii). The designated probe is vector-resolved: a
...
When the probe is run, the data select the first branch, with its
...
($|B^{\perp}|/|C^{\perp}| = 0.98$ there), inside the measured
window. Branch (ii) is ruled out: $\Phi_{768}(\sigma)$ decreases
...
($\rho \in [-1.24, -0.62]$ across every route and bin), as the
round-trip reading of the split says it must be
(Appendix~\ref{app:instrument}, \emph{the ledger's geometry}), so what
...
\section{The measured geometry: supporting panels}
\label{app:geometrypanels}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/accounts_canc.png}
\caption{\textbf{The cancellation-aware operational form
(demonstration figure; in-sample).} The exact link
(Eq.~\ref{eq:exact}) evaluated on \emph{both} shares with the
interference at the pooled constant $\bar\rho = -0.91$, drawn over
the measured curves: in-sample it reproduces the map's shape
including the $768$ in-window dip below the route's own floor, the
structure the additive two-term form cannot express. This panel is a
demonstration, not lane evidence; the lane-matched score
(\S\ref{sec:ourscored}, identical protocol with only the link
swapped) is: held-out $768$ RMSE $0.078$ vs.\ additive $0.099$ with
the dip placed at $\sigma = 0.67$ from held-out parameters alone;
leave-one-out $896$ \emph{lost} at $0.086$ vs.\ $0.077$; $1280$-tier
guard broken on $1024$ held-out ($0.199$ vs.\ $0.046$). Reported at
lane parity; not adopted --- the additive lane keeps
Fig.~\ref{fig:accounts}.}
\label{fig:accountscanc}
\end{figure}

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.49\textwidth}
\includegraphics[width=\linewidth]{figs/e250_rel.png}
\caption{Reliability of the pooled residual direction
$\widehat{R} = \widehat{(B{+}C)}$: cross-draw-set cosine per verdict
condition against the $0.5$ gate. $11$ of $12$ conditions pass; the
one failure is $768/\sigma = 0.43$, and mid-window $768$ is where
the direction is least reproducible.}
\label{fig:e250rel}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}
\includegraphics[width=\linewidth]{figs/e250_frame.png}
\caption{Matched-angle, not planar: transporting a bin's pooled leg
direction by the native anchor $\hat g(\sigma)$'s own measured
rotation before comparing across $\sigma$ changes the cosines by a
median of $-0.0005$ --- the axis rotates through the same angle as
the anchor but in its own plane, so a lookup in an anchor-normalized
frame is refuted.}
\label{fig:e250frame}
\end{subfigure}
\caption{Supporting panels for the axis-field reads of
\S\ref{sec:observations} (one adapter, three vector stores, standard
corpus; pooled directions only).}
\label{fig:e250}
\end{figure}

The two figures above carry the supporting detail for
\S\ref{sec:observations}: the lane test of the cancellation-aware
form, and the reliability and frame-transport reads that scope the
axis-field claims.
...
none safe & unsafe ($8/9$ bins) & floors $.02$--$.04$ / $.06$--$.09$ / ${\approx}\,.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
tolerance, is safe at no noise level.}
\label{fig:spectrum}
\end{figure}
...
$\delta_{\reenc}$. The transported curve decays to zero by
$\sigma \approx 0.35$
...
a nonzero curve, and it moves RMSE by at most $0.008$. All three therefore
...
slice; \citet{xiao2026spd} provide an inference-side default
...
to gradients. Our setting, however, fixes one \emph{by construction}:
the demotion recipe pays a resize--VAE-encode round trip whose latent
error has measurable per-band power $D(f)$, expressed in
Eq.~\ref{eq:spd}'s own units, so $\delta_{\reenc}(e) :=
...
$< 0.01$), predicts the measured-safe route safe only above
$\sigma \approx 0.98$ where the measurement shows it safe from $0.5$,
...
\caption{$1024{\to}896$ (the safe route).}
...
panel's route. The map replicates: on the safe route all four
...
\paragraph{Cancellation replication on the controlled adapters.}
The same two checkpoints carry the cross-adapter replication of the
cancellation geometry (\S\ref{sec:observations},
\S\ref{sec:limitations}). The vector-resolved probe of
\S\ref{sec:observations} was re-run per adapter on the $1024{\to}768$
route over the five window bins ($N{=}40$, $D{=}12$, identical probe
list; the operating adapter's reference rows from a same-environment
twin run), with criteria frozen before any adapter gradient existed:
a bin is \emph{readable} iff both legs pass the reliability gate
($\mathrm{rel} \ge 0.5$), and an adapter \emph{replicates} iff at
least $4$ of $5$ bins are readable and every readable bin has
$I < 0$, $\rho \le -0.5$, and $h(B{+}C) < \min(h(B), h(C))$. Both
adapters pass at $5$ of $5$ bins (Table~\ref{tab:e26}). One
consistency read is pre-registered with the table: from the measured
$h$-triple the law of cosines already implies a $\rho$; at $9$ of
$10$ adapter-bins the implied value is itself $\le -0.97$, so the
deep measured $\rho$ is there an arithmetic consequence of the
magnitudes and is reported as such --- the depth of enforcement is
\emph{not} claimed to scale with the perturbation. The single cell
where measured enforcement exceeds what the magnitudes force
(dirty, $\sigma = 0.7$: implied $-0.87$, measured $-0.96$) is
recorded, not generalized. $896$-route replication was not run and
remains an open cell.

\begin{table}[h]
\caption{Cross-adapter cancellation replication ($1024{\to}768$, five
window bins, frozen criteria; reenc-referenced $h(\cdot)$ units).
Every bin of both adapters is readable and passes
($I < 0$, $\rho \le -0.5$, $h(B{+}C) < \min(h(B), h(C))$): the
realized gap is the residual of the same near-cancellation on
adapters trained on disjoint data.}
\label{tab:e26}
\centering
\small
...
\toprule
 & \multicolumn{4}{c}{flat-cluster adapter} &
 & \multicolumn{4}{c}{textured-cluster adapter} \\
\cmidrule{2-5}\cmidrule{7-10}
$\sigma$ & $h(B)$ & $h(C)$ & $h(B{+}C)$ & $\rho$ &
 & $h(B)$ & $h(C)$ & $h(B{+}C)$ & $\rho$ \\
\midrule
...
\section{Render prompt for the arms figure}
\label{app:armsprompt}

The renders of Fig.~\ref{fig:kaaiyuki} share one positive prompt, a
held-out corpus-B caption, rendered at $1024^2$ with matched noise
seed across arms:
\begin{quote}
\scriptsize\texttt{safe, 2girls, kaai yuki, otomachi una, vocaloid,
zako (vocaloid), @channel (caststation), bag, black hair, black
skirt, blush stickers, bow, bowtie, collared shirt, diagonal-striped
bow, diagonal-striped bowtie, diagonal-striped clothes,
diagonal-striped neckerchief, emphasis lines, hair bobbles, hair
ornament, hands on own cheeks, hands on own face, holding, holding
phone, jitome, long sleeves, multiple girls, multiple views,
neckerchief, notice lines, one eye closed, outstretched arm, phone,
pink background, pink jacket, plaid clothes, plaid skirt, school bag,
small sweatdrop, striped bow, striped bowtie, striped clothes,
striped neckerchief, tongue, tongue out, v, v over eye, v over mouth,
white shirt, wink star}
\end{quote}
...
dense below $\sigma = 0.1$ and above $0.9$: $14$ bins in $(0, 1)$ plus
...
columns agree within CI at every route; our pre-registered
falsification criterion (all endpoint gaps ${\approx}\,0$) is not met;
