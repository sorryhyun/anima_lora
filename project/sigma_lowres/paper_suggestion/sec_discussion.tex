...
the mildest route is $\sigma \approx 0.5$ --- the $896$ route's excess
is already $+0.16$ by $\sigma = 0.04$, plateaus at $+0.22$--$0.29$
through $\sigma = 0.3$, is marginal at $0.43$ ($+0.067 \pm 0.032$),
and its means sit within $\pm 0.04$ of zero from $0.57$ on ---
$3.5\times$ the equal-power prediction. (iii)~The $2\times$ downscale
route is \emph{certifiably unsafe} across the noise axis: its paired
debiased lower bound stays above the margin in all fifteen bins (means
$+0.12$ to $+0.71$), including $+0.20 \pm 0.05$ at $\sigma = 0.94$,
where the input is ${\sim}97\%$ noise by power in every band the
coarse grid cannot represent (the earlier coarse-$\sigma$ run's one
apparent exception, $\sigma = 0.81$ with an interval widening to
include zero, does not survive the dense deterministic rerun:
$+0.22 \pm 0.05$ at $\sigma = 0.83$). There is no noise level at which
a $2\times$-downscaled latent certifies, or comes near certifying, the
...
sit at $+0.02$--$0.04$ across $\sigma \in [0.7, 0.99]$, but with
upper bounds that do not cross below the $\varepsilon = 0.02$ margin
at this $(N, D)$, and a real endpoint floor of $+0.130 \pm 0.030$
...
the margins its flattest bins do certify (${\approx}\,0.09$
simultaneous over $\sigma \in [0.7, 0.96]$) are still $4\times$ the
practical margin. Conversely the
$896$ endpoint shows a small \emph{real} gap ($+0.034 \pm 0.011$) that
single-estimate variance had buried; its safe window is the interior
$\sigma \in (0.5, 1)$, not the endpoint (Appendix
...
$\sigma \in (0.5, 1)$, certified at margin $0.09$ (simultaneous
over the seven window bins; $0.066$ pointwise); $1280{\to}1024$ (ratio
...
$896{\to}768$ (floors $0.06$--$0.13$ and $0.06$--$0.12$) have
...
every window bin's debiased mean sits within $\pm 0.04$ of zero, the
pointwise and simultaneous one-sided bounds certify $0.066$ and
$0.09$ respectively, and the strict $\varepsilon = 0.02$ margin is
\emph{not yet powered} on the interior grid
($\epsstar = 0.02$--$0.07$ per bin). Four bins' upper bounds do cross
it ($\sigma \in [0.83, 0.96]$, $\mathrm{UB}_{95} = 0.004$--$0.018$,
all on negative point estimates), but by the protocol of
\S\ref{sec:instrument} an
...
objects genuinely disagree at high $\sigma$:
