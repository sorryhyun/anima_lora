...
needs no table, because the verdict quantity self-describes it: a
route is \emph{safe at margin $\varepsilon$} exactly where its
debiased excess over the re-encoding control is indistinguishable from
zero at instrument resolution --- the one-sided non-inferiority
statement
\begin{equation}
...
\label{eq:safe}
\end{equation}
not a hard bound, with $\epsstar$ the certification resolution of
\S\ref{sec:instrument}. Measured, on the \emph{per-example} object:
