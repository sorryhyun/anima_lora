...
Finite draws are not innocuous here. Every finite-draw cosine is
attenuated below its population value, and the attenuation is
arm-dependent: per-draw gradient variance grows as the token count
falls, the MSE loss averaging over fewer elements. The natural
...
subsets and fitting $\gap(D) = \gap_\infty + c/D$ recovers the
