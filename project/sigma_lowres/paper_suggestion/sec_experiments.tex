...
tell the two apart. We observed this bias live and large at our
operating point: a variance back-of-envelope predicts spurious
iso-direction
...
two-estimate structure the native floor has; the floor is precisely the
...
power available to certify it. We fix the strict margin at
...
there is stated at the smallest margin its bounds do certify. A route
is
\emph{certified safe at margin $\varepsilon$} in a bin where
...
($0.09$ for the interior windows of \S\ref{sec:map});
...
Throughout, \emph{safe} means the certification of
...
there, so two further probes close the step. The \emph{x-zero}
probe's debiased
draw-limit gaps equal the endpoint gaps at every route
(Table~\ref{tab:floor}), and a \emph{target-strength sweep},
re-running the endpoint with target $\epsilon - \alpha x$,
$\alpha \in [0, 1]$, finds the paired debiased excess
$\alpha$-flat at the well-conditioned anchors: no resolvable
target-content
...
as the angular estimand reads it. We provide the sweep's anchors and
the parallel-landing decomposition,
...
(Eq.~\ref{eq:twoterm}) carries one sharp prediction: the positional
...
relative phase geometry match native. At the $\sigma{=}1$ endpoint this \emph{erases} the $768$ floor
