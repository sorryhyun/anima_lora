...
safe on the interior window $\sigma \in (0.5, 1)$, certified
...
is \emph{certifiably unsafe} in all fifteen bins. Debiasing itself moved
...
The spectral account's prediction is monotone: the mismatch is
...
curves are near zero exactly where the measured curves peak (RMSE
...
\paragraph{Ours, the data branch: a route-uniform residual
mismatch.}
...
U-shaped total gradient norm, yields the measured interior-peaked
...
single-artist corpora ($60$/$15$ training images). Training is
bit-exact deterministic (an identical retrain reads $1.000$); the
non-det.\ row retrains native at the same seed \emph{without}
deterministic kernels (one twin pair per corpus), so it reads
run-to-run kernel chaos alone. \emph{late}: demotion only in the last
...
(\S\ref{sec:reduction}) carries one sharp prediction: the positional
part is a coordinate-system artifact, so an origin-side intervention
should erase it wherever the data branch is inactive. Evaluate the
demoted grid at positionally-interpolated fractional rotary
coordinates (patch $i$ at $i \cdot H_{\src}/H_{\dem}$), making its
relative phase geometry exactly match native. At the $\sigma{=}1$ endpoint this \emph{erases} the $768$ floor
...
non-positional graph residue, and the capacity governor attaches to
it. The erasure is \emph{not} a
