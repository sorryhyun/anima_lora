...
(\S\ref{sec:instrument}), then score both accounts against the
measured curves --- the spectral one at the boundary and the curve
level, ours term by term (\S\ref{sec:scored}); the per-route map those
curves certify is read in \S\ref{sec:map}, where it becomes the
deployed artifact, and the held-out prediction protocol, its
pre-registered gates, and the structural failures it exposes are
quantified in Appendix~\ref{sec:headtohead}. Each subsection states
