...
\caption{\textbf{The existing account against the measurement.} Left
three panels: measured debiased gap curves (Eq.~\ref{eq:gapdef})
against the spectral account transported through our bridge --- the
diagonal model's destroyed-band mean-residual mismatch on the measured
spectrum, through the measured $\|\bar g_{\src}(\sigma)\|$ with a
single gain calibrated on the one safe route and no floor (the family
expresses none) --- gated at $t^{*}(\delta)$ (dashed verticals) for the
equal-power slice, SPD's default $\delta = 0.01$, and the measured
anchor $\delta_{\reenc}$. The transported curve dies by
$\sigma \approx 0.35$ on every route, so the latter two gates act on an
already-zero curve and coincide. Right: the boundary family
$t^{*}(\delta)$; maximum route spread $0.125$ in $\sigma$ over
$\delta \in [10^{-3}, 0.5]$. Unit honesty: these curves are \emph{the
spectral account read through our bridge}, since it emits residual
units only (\S\ref{sec:specscored}).}
\label{fig:e83}
\end{figure}
...
\caption{\textbf{Our account against the same measurement}, under the
headline exact angular link (Eq.~\ref{eq:exact}; three-form comparison
in Appendix Fig.~\ref{fig:e51280}). Left: $1024$-tier predictions with
zero per-route freedom --- the pre-registered held-out route
$1024{\to}768$, and $1024{\to}896$ under the post-hoc leave-$896$-out
re-derivation (so labeled; governors from $512{+}1120$ alone). Dashed:
each route's predicted floor $F(n)$, so dashed-to-solid is the data
term. Bands: $68\%/95\%$ full-pipeline bootstrap ($B{=}1000$;
Appendix~\ref{app:instrument}). The $768$ mid-$\sigma$ dip sits outside
the $95\%$ band and is the reduction's one structural failure
(\S\ref{sec:headtohead}). Right: predicted vs measured across all five
routes and bins; filled: held-out, open: fit routes.}
\label{fig:e5}
\end{figure}

Figures~\ref{fig:e83} and~\ref{fig:e5} state the comparison before the
...
clean division of labor, and together they decide the equal-ratio row
of Table~\ref{tab:accounts}. \emph{Absolute size, not ratio, sets the
...
measured route-uniform $m(\sigma)$ and each run's own
...
Fig.~\ref{fig:e5}). (i)~Held-out $1024{\to}768$: RMSE $0.093$,
...
(Fig.~\ref{fig:e5}): a positive two-term form cannot produce a gap
