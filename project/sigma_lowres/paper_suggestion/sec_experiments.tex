...
count falls, the MSE loss averaging over fewer elements. And the
natural estimate of Eq.~\ref{eq:gapdef}, a redraw floor minus a
single demoted-arm cosine, is asymmetric: the floor compares
\emph{two} finite-draw estimates of the native gradient where each
demote arm contributes one. An iso-direction null, in which demotion
changes \emph{nothing} but the variance, therefore still produces a
positive gap that \emph{grows as the target grid shrinks}, the same
...
operating point; measured magnitudes and the $c/D$ decay are given in
...
${\approx}\,1$. That is, the population per-bin native gradient is a
\emph{fixed direction}, and the entire redraw floor is finite-draw
noise; validation numbers are given in Appendix~\ref{app:instrument}.
