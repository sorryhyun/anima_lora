...
\S\ref{sec:reduction} is concrete here: the MSE loss averages over
fewer elements on the coarser grid, and the natural estimate of
Eq.~\ref{eq:gapdef}, a redraw floor minus a single demoted-arm
cosine, adds an asymmetry of its own: the floor compares \emph{two}
finite-draw estimates of the native gradient where each demote arm
contributes one. We observed the resulting bias, whose
floor-mimicking signature \S\ref{sec:reduction} derives, live and
large at our
...
at margin $0.09$ simultaneously over the seven window bins; above
$\sigma = 0.94$ the bins are underpowered at margins below
${\approx}\,0.05$ and the $\sigma{=}1$ endpoint carries a small real
gap (below), so the certified condition is $\sigma \in (0.5, 0.94]$
(\S\ref{sec:limitations}).
$1024{\to}768$ and $896{\to}768$ (floors $0.06$--$0.09$) certify no
...
account does not predict (the vector-resolved probe traces it to
...
relative phase geometry match native (a raw-estimator probe;
\S\ref{sec:limitations}). At the $\sigma{=}1$ endpoint this \emph{erases} the $768$ floor
