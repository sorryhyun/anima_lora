...
(\S\ref{sec:instrument}), score both accounts against the measured
curves --- the spectral one at the boundary and the curve level, ours
term by term (\S\ref{sec:scored}) --- read the per-route map those
curves certify (\S\ref{sec:verdict}), and put both accounts head to
head as predictors of held-out routes
(\S\ref{sec:headtohead}). Each subsection states the
...
record --- \S\ref{sec:verdict} then sets out in full. Four features of
...
\paragraph{The spectral account, at the boundary level.}
...
\paragraph{And at the curve level: $\delta$ is inert.}
...
The curves both accounts were just scored on carry a verdict of their
own, read per bin. Figure~\ref{fig:verdict} gives the debiased map for
$1024$-tier natives demoted to $896/768/512$ (per-bin numbers: Appendix
