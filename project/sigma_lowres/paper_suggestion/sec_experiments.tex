...
reduction's one structural failure (Appendix~\ref{sec:headtohead}). Bottom
...
term by term (\S\ref{sec:scored}) --- and read the per-route map those
curves certify (\S\ref{sec:verdict}); the held-out prediction protocol
behind the figure's overlays, its pre-registered gates, and the one
structural failure it exposes are quantified in
Appendix~\ref{sec:headtohead}. Each subsection states the
...
population directions, so every verdict needs a stated resolution. For
a route whose true excess is zero, the paired estimate $\bgap$
fluctuates with standard error $\mathrm{SE}(N, D, \sigma\text{-bin})$;
its one-sided 95\% upper bound,
$\epsstar(N, D, \text{bin}) = 1.645\,\mathrm{SE}(N, D, \text{bin})$,
is the \emph{certification resolution} --- the smallest margin
certifiable even for a truly safe route (the formal safety statement
built on it is stated with the map, Eq.~\ref{eq:safe} in
\S\ref{sec:map}). ``No certifiable window'' means a route's upper
bounds do not cross below the margin at the current $(N, D)$;
``certifiably unsafe'' means its lower bound stays above it. At the
verdict grid's operating point ($N{=}40$, $D{=}8$/bin) the bin-level
$\epsstar$ is $0.03$--$0.08$; endpoint-mode draw sweeps ($D$ up to
$64$) tighten it to
...
$2\times$-downscaled latent yields the native gradient direction.
