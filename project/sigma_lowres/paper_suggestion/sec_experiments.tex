...
which is the deployed artifact (\S\ref{sec:map}), and then score both
accounts against the same curves: the spectral account at the
boundary and the curve level, ours term by term (\S\ref{sec:scored}).
The held-out prediction protocol, its pre-registered gates, and the
structural failures it exposes are quantified in
Appendix~\ref{sec:headtohead}. Each subsection states the prediction
it discharges against criteria frozen before the runs; several probes
double as falsifiers of our own account's assumptions.
...
similarity of the flattened accumulated gradients. The arms are: a
...
\paragraph{Finite-draw attenuation.}
Finite draws are not innocuous here. The arm dependence of
\S\ref{sec:reduction} is concrete: per-draw gradient variance grows
as the token count falls, the MSE loss averaging over fewer
elements. The natural
estimate of Eq.~\ref{eq:gapdef}, a redraw floor minus a single
demoted-arm cosine, is also asymmetric: the floor compares \emph{two}
finite-draw estimates of the native gradient where each demote arm
contributes one. An iso-direction null, in which demotion changes
...
tell the two apart. The mechanism is live and large at our operating
point: a variance back-of-envelope predicts spurious iso-direction
endpoint gaps of ${\approx}\,0.02/0.05/0.15$ for $896/768/512$, and
the uncorrected $1024{\to}896$ endpoint gap indeed decays as
...
First, \emph{self-floors with attenuation correction}: every arm runs
a second independent draw set $g_a'$, giving the per-arm self-cosines
that Eq.~\ref{eq:debias} consumes, so that each arm has the same
two-estimate structure the native floor has; the floor is exactly the
source arm's self-cosine,
$\cos_{\mathrm{floor}} = \cos_{\mathrm{self},\src}$, and the debiased
gap $\dgap$ is computed per image and bin. Second,
\emph{draw-count extrapolation}: the attenuation is the $c/D$ decay
just measured, so fitting $\gap(D) = \gap_\infty + c/D$ over nested
draw subsets recovers the draw-limit gap directly. The two are mutually checking, and the check
passes: debiased fits come out $D$-flat ($|c| \le 0.05$ on the $512$
route, against $c \approx +0.29$ uncorrected), and the nested sweep
extrapolates the native redraw floor to a self-cosine of $1.005$
(bootstrap $95\%$ CI $[0.994, 1.016]$). That is, the population
per-bin native gradient is a \emph{fixed direction}, and the entire
redraw floor is finite-draw noise; remaining validation detail is in
Appendix~\ref{app:instrument}.
...
stays above it. At the verdict grid's operating point of $N{=}40$ and
$D{=}12$ per bin, the bin-level $\epsstar$ is $0.02$--$0.07$, so
interior verdicts below are stated at the margins they do certify;
...
sensitive, so all pairings are computed within one run. Every number
in the main text is debiased. Four secondary probes (the $1280$-tier
...
$\pm\epsstar$ certification resolution (\S\ref{sec:instrument}). The
...
alike (Appendix Fig.~\ref{fig:e83}); and ours under the exact angular
...
the $95\%$ band; these are the two edges of the reduction's domain
...
Throughout, \emph{safe} means exactly the certification of
Eq.~\ref{eq:safe}, namely gap excess over the re-encode control below
the stated margin, on the \emph{per-example} object: the sense in
which a demoted gradient is indistinguishable from a native one.
Route by route: $1024{\to}896$ (floor $0.02$--$0.04$) is
\textbf{safe} on the interior window $\sigma \in (0.5, 1)$, certified
at margin $0.09$ simultaneously over the seven window bins.
$1024{\to}768$ and $896{\to}768$ (floors $0.06$--$0.13$) certify no
window at the strict margin, but $1024{\to}768$ retains a narrow
\textbf{low-excess window} $\sigma \in (0.65, 0.95)$, with paired
excess $+0.02$--$0.04$ per bin, certified at margin $0.09$
simultaneously over its four window bins ($0.072$ pointwise), atop a
floor no noise level removes; this is the window \S\ref{sec:trainer}
stacks as a second route. Any route to $512$ (floor ${\approx}\,0.30$)
is \emph{certifiably unsafe} in all fifteen bins. Safety is a
per-route boundary, jointly determined by the ratio, which acts
through $a_e$, and by absolute target size, which acts through the
floor. Debiasing itself moved
verdicts in both directions, and the two revisions against the
historical record are part of the result: roughly half the $768$
...
$896$ endpoint shows a small \emph{real} gap ($+0.034 \pm 0.011$)
that single-estimate variance had buried.
...
objects genuinely disagree at high $\sigma$: pooled-arm probes at
pool size $4$ collapse the $1024{\to}896$ and $1024{\to}768$ excesses
to ${\approx}\,0$ over the upper bins, so the batch-aggregate object
SGD follows is more forgiving here than the per-example read we
publish as the verdict. A debiased verdict grid with pooled
self-floors (pool $4$, $N{=}40$; paired arm$-$reenc gaps at
$B \in \{1, 4, 40\}$) bounds the coherent drift no batch size
removes: the $a + b/B$ fit's intercept is ${\leq}\,0.02$
($1024{\to}896$) and ${\leq}\,0.03$ ($1024{\to}768$) at
$\sigma \geq 0.44$, but persists over the lower bins
($+0.04$--$0.09$, $+0.05$--$0.28$, and $+0.15$--$0.60$ for
$896/768/512$) — aggregation buys safety only where the per-example
map is already near-safe. The small real $896$ endpoint gap
($+0.042$) averages out in the aggregate ($+0.001$). The shipped
trainer's operating point is batch~$1 \times$ accumulation~$1$, so
the per-example map is not merely the worst case — it \emph{is} the
shipped-trainer object; the pooled maps bound users who raise the
batch or accumulation.
...
and then at the curve level; ours follows, term by term.
...
model's approximation error, not model error alone) shows a strong,
...
and that all route identity lives in $J$. This was a pre-registered
contrast that would equally have falsified our own assumption~(iv);
...
spectral crossover. The denominator then does real work: the total
gradient norm is U-shaped in $\sigma$ ($2.6 \to 0.4$ at
...
consistency analysis and is labeled as such; all safety criteria stay
in cosine units, since direction is what a normalized optimizer
consumes.
...
$\hat g_{\src}$, to which the cosine gap is blind
(Eq.~\ref{eq:exact}); the endpoint gap is therefore the graph floor
as the angular estimand reads it (parallel-landing decomposition,
with the $\kappa_\parallel/\kappa_\perp$ numbers, in
...
\caption{Weight-space footprint vs.\ the seed lottery:
$\cos(\Delta W_{\text{arm}}, \Delta W_{\text{native}})$ at matched
training seed, mean $\pm$ SD over three seed pairs, on two
single-artist corpora ($60$/$15$ training images). The cross-seed row is the same
cosine between native endpoints differing \emph{only} in training
seed; an arm above it displaces the endpoint \emph{less} than a seed
change. Training is bit-exact deterministic (an identical retrain
reads $1.000$); the non-det.\ row retrains native at the same seed
\emph{without} deterministic kernels (one twin pair per corpus), so it
reads run-to-run chaos alone. \emph{late}: demotion only in the last
$75\%$ of steps; the stacked row adds a second certified route
($1024{\to}768$ on $0.65{<}\sigma{<}0.95$, last $50\%$).
...
\mathrm{Resid}_e$.}
The prediction of \S\ref{sec:reduction}, that the positional share is
a coordinate-system artifact, is testable by an origin-side
...
training lever, since with image content in the input the stretched
