...
overlays, its pre-registered gates, and the one structural failure it
...
A route is accordingly \emph{safe at margin $\varepsilon$} exactly
where its debiased excess is indistinguishable from zero at instrument
resolution --- the one-sided non-inferiority statement
\begin{equation}
...
\label{eq:safe}
\end{equation}
not a hard bound, with $\epsstar$, the zero-excess estimate's one-sided
95\% upper bound, the \emph{certification resolution} --- the smallest
margin certifiable even for a truly safe route (the measured map of
\S\ref{sec:map} is stated in exactly these terms). ``No certifiable
window'' means a route's upper
...
Eq.~\ref{eq:posterior} intrinsic posterior bias plus the frozen
