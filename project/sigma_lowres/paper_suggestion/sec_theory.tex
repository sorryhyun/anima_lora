...
pure-noise answer $\epsilon^{(\omega)}$; $\delta$ is the account's one
free parameter, whose instantiations are explored in
...
branch, gated at Nyquist, and sets the rest to zero --- the three
limits fixed there are exactly the discarded terms read back: the
graph branch is a property of the two grids, referencing neither
$\sigma$ (limit~c) nor their ratio (limit~b), and a two-term gap
admits no single-parameter ordering (limit~a). One discarded term
matters later: the regression target carries the clean image at unit
weight at every $\sigma$, so even at $\sigma{=}1$ the mean residual
retains an image-dependent survivor, $x - \mathbb{E}[x \mid c]$
...
The geometry is pure bookkeeping, so the displayed forms --- and the
$\kappa$ notation they are stated in --- live in
...
fact the endpoint probes of \S\ref{sec:ourscored} lean on. Expanding
at small perturbation and substituting Eq.~\ref{eq:branches} gives the
\emph{four-term angular expansion} (Eq.~\ref{eq:fourterm}), derived as
unconditionally as the exact form itself: writing $u^{\perp}$ for the
component of $u$ orthogonal to $\bar g_{\src}$, a data share
$S_e = \|B_e^{\perp}\|^2 / (2\|\bar g_{\src}\|^2)$, a graph share
built likewise from $C_e^{\perp}$ (both non-negative), a
projected interaction $I_e$ that can carry either sign (a fact that
matters in Appendix~\ref{sec:headtohead}), and a remainder $R'_e$.
...
denominator share the residual's scale), so the graph share is flat in
$\sigma$ --- the per-route \emph{floor} of Eq.~\ref{eq:twoterm};
...
exact Cauchy--Schwarz bounds in the retained shares (the interaction
by their geometric mean, the remainder entering at half power in its
own share), so (ii) carries independent content only near share
crossover (Appendix~\ref{app:geometry}). Applied,
\begin{equation}
\gap_e(\sigma)
\;\approx\;
...
The ledger: Eqs.~\ref{eq:exact} and~\ref{eq:fourterm} are
unconditional, Eq.~\ref{eq:twoterm} is conditional on the four
assumptions, and the coefficients --- $a_e$, $\|\Delta \bar r(\sigma)\|$,
$\|\bar g_{\src}(\sigma)\|$, and each route's floor --- are measured. The account,
with an outcome matrix that included a kill switch on the graph term
(all endpoint gaps ${\approx}\,0$), was pre-registered before the
discriminating runs.
...
discretization-invariant) --- the two parts $\mathrm{RoPE}_e$ and
$\mathrm{Resid}_e$ written into Eq.~\ref{eq:twoterm}. The split
yields one sharp prediction: the positional share is a \emph{coordinate-system
...
calibrated, not derived; we claim no closed form for the floor.
