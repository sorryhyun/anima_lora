...
$F_e$ built likewise from $C_e^{\perp}$ (both non-negative), a
...
Eq.~\ref{eq:spd} is Eq.~\ref{eq:fourterm} with $F_e = I_e = 0$ and
...
\;+\; \underbrace{\mathrm{RoPE}_e + \mathrm{Resid}_e}_{\text{graph term }\Floor_e},
\label{eq:twoterm}
\end{equation}
...
$\|\bar g_{\src}(\sigma)\|$, $\Floor_e$ are measured. The account,
