...
parameter can order all routes (\S\ref{sec:sidebyside}).
...
which decodes and re-encodes the native latent at native resolution: it pays the VAE round trip that demotion necessarily pays,
...
\paragraph{When a band stops mattering.}
The noising is linear, so it acts band by band:
$z_\sigma^{(\omega)} = (1-\sigma)\,x^{(\omega)} +
\sigma\,\epsilon^{(\omega)}$, and the velocity target splits as
$v^{(\omega)} = \epsilon^{(\omega)} - x^{(\omega)}$. Under the premise
that each clean-latent band is independently Gaussian,
$x^{(\omega)} \sim \mathcal{N}(0, P_\omega)$ with $P$ the clean-latent
power spectrum, the \emph{Bayes-optimal} per-band velocity prediction
$v^{*(\omega)} = \mathbb{E}\big[v^{(\omega)} \mid z_\sigma\big]$ is a
per-band linear shrinkage of the input, and as $\sigma$ grows the
band's posterior collapses to its prior: the optimal prediction tends
to the pure-noise answer $\epsilon^{(\omega)}$. The crossover
$t_\omega$ makes this quantitative --- for all $\sigma \ge t_\omega$
the prediction is within a tolerance $\delta$ of pure noise,
$\mathbb{E}[|v^{*(\omega)} - \epsilon^{(\omega)}|^2] \le \delta$. The
tolerance is the account's one free parameter, setting how strict
``stops mattering'' is; choosing $\delta = (1+P_\omega)/2$ recovers
...
Writing $S_e(\sigma)$ for that destroyed-band contribution to the
distance of Eq.~\ref{eq:gapdef}, the implied gradient gap is $S_e$
gated at the Nyquist band's activation time:
\begin{equation}
...
\label{eq:spd}
\end{equation}
Under the same premise the account also fixes the residual level: with
...
destroyed-band energy (the Gaussian posterior mean is the classical
per-band Wiener shrinkage, \citealp{wiener1949}).
...
so the account is structurally blind to any absolute-size effect. \textbf{(c)~No floor}: for every route and
