...
(\S\ref{sec:spectral}), and our own (\S\ref{sec:ouraccount}). The two
...
with proportionally fewer spatial tokens; the downscale acts in pixel
...
removes, rather than aliases, whatever lies above. Idealizing the VAE
round trip away, whose residue is exactly what the $\reenc$ control
of \S\ref{sec:estimand} pays, demotion is an ideal low-pass:
\begin{equation}
x_{\dem}^{(\omega)} \;=\;
\begin{cases}
...
per-band velocity prediction sits within a tolerance $\delta$ of the
pure-noise answer $\epsilon^{(\omega)}$. The tolerance $\delta$ is
the account's one free parameter; its instantiations are explored in
Appendix~\ref{app:spectral}, and no conclusion below hinges on its
choice.
...
Our claim is therefore not that $S_e$ cannot be estimated, for it can
be, one way or another, but that a gap estimate built from it is
...
however $S_e$ is computed, and each is a point at which the account
can be falsified without arguing about $\delta$.
...
\textbf{(c)~No floor}: for every route and every $\delta$ the
predicted gap vanishes above a finite boundary, so no
$\sigma$-independent term is expressible.
...
estimand is built from noise means, so the relevant
residual object is the \emph{mean residual} at fixed query,
$\bar r_a(\sigma) = \mathbb{E}_\epsilon[\hat v_\theta - v]$, under the
...
branch, gated at Nyquist, and sets the rest to zero. The three limits
fixed there are exactly the discarded terms read back: the graph
branch is a property of the two grids, referencing neither $\sigma$
(limit~c) nor their ratio (limit~b), and a two-term gap admits no
single-parameter ordering (limit~a). One discarded term matters
later: the regression target carries the clean image at unit weight
at every $\sigma$, so even at $\sigma{=}1$ the mean residual retains
an image-dependent survivor, $x - \mathbb{E}[x \mid c]$
(Appendix~\ref{app:posterior}); whether it is resolvable in the
angular estimand is an empirical question, discharged in
\S\ref{sec:ourscored}.

\paragraph{The angular reduction.}
It remains to read Eq.~\ref{eq:branches} in the units of the estimand.
The geometry is pure bookkeeping, so the displayed forms, together
with the $\kappa$ notation in which they are stated, are deferred to
...
$|I_e|$ small against the retained shares;
