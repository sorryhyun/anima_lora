...
severity. Everything else is discriminating, and fixed by the
\emph{form} of Eq.~\ref{eq:spd} against Eq.~\ref{eq:twoterm} before any
gradient is measured. They disagree about \emph{what orders routes}
(the downscale ratio alone, against the ratio through the amplitude
$A_e$ \emph{and} the absolute target token count through the floor
$\Floor_e$; \S\ref{sec:ourscored}); about \emph{what is free} (one
tolerance $\delta$ shared by every route, against per-route
$A_e, \Floor_e$ generated by two governors; \S\ref{sec:headtohead});
about the \emph{endpoint} ($\gap_e \to 0$ on every route, against
$\gap_e \to \Floor_e > 0$; \S\ref{sec:ourscored}); about \emph{curve
shape} (a gated step at $t_{\omega_e}$, against
$\|\Delta \bar r(\sigma)\|/\|\bar g_{\src}(\sigma)\|$ carried through
Eq.~\ref{eq:exact}; \S\ref{sec:specscored}); about the \emph{cross-grid
residual} $\Delta \bar r$ (confined to the destroyed band and ordered
by its energy, against a norm shared by all routes with route identity
living in $J$; \S\ref{sec:ourscored}); and about \emph{equal-ratio
routes at different absolute sizes} (identical, against same $A_e$ and
different $\Floor_e$; \S\ref{sec:ourscored}). Each disagreement is an
experiment in \S\ref{sec:evidence}. Two of them are decidable without
fitting anything: if a route's gap survives at $\sigma = 1$, where the
input is pure noise on every grid, no member of the spectral family can
produce it; and if two routes of the \emph{same ratio} at different
absolute sizes differ, ratio is not the whole governor.
