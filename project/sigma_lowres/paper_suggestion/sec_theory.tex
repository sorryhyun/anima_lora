...
cosine, the object SGD actually follows). These are different
estimands --- the aggregation operator is part of the estimand ---
and the per-example map is \emph{not} a lower bound for larger
batches: to second order the batch-aggregate distance is a coherent
intercept that no batch size removes plus a $1/B$ term
(Appendix~\ref{app:notation}), and monotone improvement with $B$
would require an iid zero-mean disagreement model we have not
verified, so no coefficient fitted on one object is guaranteed to
transfer to the other. \S\ref{sec:map} publishes one map for each.
...
$x^{(\omega)} \sim \mathcal{N}(0, P_\omega)$, where $P_\omega$ is the
band's clean-latent power --- the power spectrum $P$ evaluated at
$\omega$ --- the \emph{Bayes-optimal} per-band velocity prediction
