...
\paragraph{The residual factor has an exact posterior form.}
The data branch's $\sigma$-curve ingredient $\Delta\bar r$ is not a
black box. For the population-optimal (Bayes) velocity field
$v_e^{*}(z, \sigma, c) = \mathbb{E}[v \mid z, c] =
\big(z - m_e(z, c)\big)/\sigma$, with
$m_e(z, c) = \mathbb{E}[x \mid z, c]$ the grid-$e$ posterior mean, the
fixed-image mean residual evaluates exactly to
\begin{equation}
\bar r_e^{*}(\sigma; x)
\;=\;
\frac{x \;-\; \mathbb{E}_{z_\sigma \mid x}\big[m_e(z_\sigma, c)\big]}{\sigma},
\qquad
\bar r_e^{*}(1; x) \;=\; x - \mathbb{E}[x \mid c],
\label{eq:posterior}
\end{equation}
so the cross-grid mismatch is a difference of two posterior
reconstruction-bias operators, and the target-mediated $\sigma{=}1$
survivor of the previous paragraph is exactly the image's deviation
from the caption-conditional prior mean (derivation, score form, and a
Gaussian closure: Appendix~\ref{app:posterior}). Two consequences are
used below. First, MSE optimality zeroes the residual conditional on
the \emph{input}, not the image:
$\mathbb{E}[v^{*} - v \mid z, c] = 0$ does not imply
$\mathbb{E}_\epsilon[v^{*} - v \mid x, c] = 0$, and
Eq.~\ref{eq:posterior} is generally nonzero --- so the measured
residual of a trained network splits exactly as
$\bar r_{\theta,e} = \bar r_e^{*} +
\mathbb{E}_{z_\sigma \mid x}[q_{\theta,e}]$, intrinsic posterior bias
plus approximation error
($q_{\theta,e} = \hat v_{\theta,e} - v_e^{*}$), a
\emph{mean-residual} mismatch rather than a model-error difference
alone. Second, the noising path fixes only how the posterior operators
are smoothed in $\sigma$; a numerical curve requires a model of the
grid-conditional clean-latent distribution, which is not analytically
available. In these terms the spectral account's residual-level form
(\S\ref{sec:spectral}) is the \emph{diagonal-Gaussian} closure of this
identity, and the measured $\|\Delta \bar r(\sigma)\|$ consumed by
assumption (iv) below is its \emph{empirical} closure --- an estimate
of a defined functional of the two posterior operators, not an
unconstrained fitted shape.
