...
appear, and abbreviated to the target edge alone --- an edge value,
never an arm label, so it always instantiates numerically ($d_e$,
$\Floor_{1120}$, $\mathrm{Resid}_{768}$) --- wherever the source edge is
fixed by context, as it is in almost every measured statement. The
...
\paragraph{The two aggregations at second order.}
The batch-size decomposition asserted in \S\ref{sec:estimand} is the
following. Writing each image's demotion-induced gradient
disagreement as a coherent mean $b$ plus a zero-mean idiosyncratic
deviation with covariance $\Sigma_\eta$, the batch-aggregate distance
...
out) plus a $1/B$ term. The intercept is exactly the coherent drift
that the intercept-plus-$1/B$ fit across pool sizes
...
theoretical status --- an empirical closure of
...
RMSE bar --- so second-order data-only structure recovers the shape
but not the level, and the essential missing ingredients are the ones
the identity names: caption-conditioning (the closure is
...
honest closure. A theory--instrument
comparison must also reproduce the instrument's estimand --- the
split-half-corrected \emph{relative} $L_2$ distance of
\S\ref{sec:instrument}, i.e.\
$\sqrt{\mathbb{E}\|\bar r_d - A_e \bar r_s\|^2}$ normalized by the
mean of the two arms' root-mean-square norms --- not a raw mismatch
norm.
...
interaction pass
through untouched --- the control has nothing to cancel them with.
...
\begin{equation}
|I_e| \;\le\; 2\sqrt{S_e\,F_e}\,,
\qquad
\big|R'_e\big| \;\le\; 2\sqrt{\big(S_e + F_e + I_e\big)\,\rho_e}
\;+\; \rho_e
...
exceed the retained sum ($2\sqrt{S_eF_e} \le S_e + F_e$) and is
automatically negligible wherever one share dominates the other, so
assumption (ii) carries independent content only near share crossover
--- which is exactly where Appendix~\ref{sec:headtohead} finds it failing. The
second says the first-order part of the remainder enters at half power
--- one factor of $\sqrt{\rho_e}$ per retained share --- so assumption
(i) reduces to the branch-level statement that $R_e$ is small against
$B_e$ and $C_e$, plus the small-$\kappa$ domain for the tail. The
...
with $\sigma$ through the network input --- and the image- and
...
\paragraph{Finite-draw bias, measured.} The bias mechanism of
\S\ref{sec:instrument} is large at the probe's operating point: a
variance back-of-envelope predicts spurious iso-direction endpoint
gaps of ${\approx}\,0.02/0.05/0.15$ for $896/768/512$, and the
$1024{\to}896$ endpoint gap indeed decays as
${+}0.100 / {+}0.035 / {-}0.016$ at $D = 4/8/16$, a clean $c/D$ decay
with fitted $c \approx 0.5$. After debiasing, the fits come out
$D$-flat ($|c| \le 0.05$ for the $512$ route, against
$c \approx +0.29$ uncorrected). The native floor makes the point
sharpest: at $D{=}8$--$16$ the uncorrected endpoint self-cosine reads
$0.79$--$0.85$, but the nested sweep ($D = 4 \ldots 64$) extrapolates
it to $1.005$ (bootstrap 95\% CI $[0.994, 1.016]$) --- the entire
redraw floor is finite-draw noise.
...
--- the low-norm amplification of \S\ref{sec:ourscored} surfacing along
the $\alpha$ axis. The $512$ route's sweep anchors are
...
candidate for cross-band coupling exists --- the flow posterior
...
premise --- but its rank-one common-mode version is excluded by the SVD
read above.
...
curves, refit the exact-link $(a_e, \Floor_e)$ on every fit route,
...
committed floor law's positive-floor filter ($\Floor > 0.005$): in
$41\%$ of draws the resampled $1120$ floor clears the filter and the law
re-anchors on the $(896, 1120)$ legs rather than $(512, 896)$ ---
branch variability the bands therefore contain, and the reason
...
through $512{+}1120$, whose fragile $\Floor_{1120}$ leg enters the log
...
pipeline cost is a minor share of the data intervention --- closing the
one open item on the floor ledger's first share, where the re-encoding
control (native decode$\to$re-encode) is a \emph{proxy} for the pipeline
cost demotion actually pays.
...
initially read as refuting a rotary mechanism --- an instructive error,
since landing-side uniformity cannot localize \emph{origin}: a
...
on the one safe route and no floor --- the family expresses none),
...
Fig.~\ref{fig:spectrum}). The equal-power slice
...
the uncorrected counterpart of the verdict map, and its last two rows
are the estimator context plotted in Fig.~\ref{fig:estimator}.
...
draws; trimmed per \S\ref{sec:instrument}, retaining $35$--$40$ images
per cell). Bin-level $\epsstar$ at this $(N, D)$ is $0.03$--$0.08$.}
