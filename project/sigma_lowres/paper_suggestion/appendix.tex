...
\section{Notation and estimand fine print}
\label{app:notation}
...
These are different estimands, and no coefficient fitted on one is
guaranteed to transfer to the other; \S\ref{sec:map} publishes one map
for each and names the $a + b/B$ fit as the designated readout for the
intercept.
...
Write $g = \bar g_{\src}$, $G = \|g\|$, $\hat g = g/G$, and
...
$m$ is read net of re-encode; the graph share and the interaction pass
...
--- which is exactly where \S\ref{sec:headtohead} finds it failing. The
...
held-out in \S\ref{sec:headtohead}. The geometry above fixes only how a
...
m(\sigma)$, which would turn the data term into an inequality in the
observed $m$ given a $\sigma$-uniform bound on the mean Jacobian's top
...
the $\alpha$ axis. In the parallel-landing read, the per-image
orthogonal loading is ${\sim}10\times$ the aggregate's:
image-specific directions cancel in the mean, the same motif as the
residual-direction read below.

\paragraph{Direction-resolved mismatch (behind
\S\ref{sec:ourscored}).} The route-uniformity of $m(\sigma)$ is a
...
functional forms for the data term of \S\ref{sec:headtohead}
(fitted power, derived quadratic, exact angular link), predicted from
...
why the exact link headlines in Fig.~\ref{fig:e5}.}
\label{fig:e51280}
\end{figure}
...
curves (pre-debiasing paired reads; caveat in \S\ref{sec:limitations}),
the route-uniform amplitude $m(\sigma)$ from the residual probe, and
...
with profile-likelihood uncertainties on $c_e$. Oracle baselines are
...
(Fig.~\ref{fig:e5}).} The bands are a full-pipeline image bootstrap
($B{=}1000$, fixed seed; $981$ draws kept): resample images with
replacement within each run ($N{=}40$ / $N{=}24$), re-bin the paired
curves, refit the exact-link $(c_e, \Floor_e)$ on every fit route,
re-derive the ratio governor and floor law, re-predict. $\bar m$ and
...
one. The $896$ curve in Fig.~\ref{fig:e5} is the post-hoc
...
exact by construction. One unit-honesty caveat on
...
one caveat on the floor ledger's first share, where the re-encoding
...
\S\ref{sec:headtohead}: per (route, $\sigma$-bin) orthogonal
...
Fig.~\ref{fig:gapcurves}, the paper's verdict map. The remaining tables
