...
None of this work analyzes the gap at \emph{training} time --- whether a
downscaled step delivers the native gradient direction under an
unchanged objective. That is the route this paper takes. What a training step
consumes is the expected adapter gradient, and \emph{demotion} --- our
term for the training-time substitution of a downscaled, re-encoded
latent for the native one --- perturbs more than the network's input:
...
does output-level evidence settle the matter: a small quality-score
difference does not transfer to a small difference in what a training
step \emph{learns}. Measuring it honestly is, unexpectedly, a metrology
problem: the natural gap estimator (a redraw floor minus a single
demoted-arm cosine) is biased by finite-draw variance, and the bias
grows as the demoted grid shrinks, \emph{manufacturing} exactly the
token-count-ordered floor signature under test. Every claim below is
stated in debiased units, at the instrument's measured resolution.
...
predictive tool} (\S\ref{sec:evidence}). Measured with per-arm
