...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        attn_bias = relative_position_bias.unsqueeze(0).to(
            q.dtype
        )  # 1, nH, N, N (broadcasts over B_)
        ...
        if mask is not None:
            ...
            attn_bias = attn_bias.unsqueeze(0) + mask.view(1, nW, 1, N, N).to(
                q.dtype
            )  # 1,nW,nH,N,N
            x = F.scaled_dot_product_attention(
                q, k, v, attn_mask=attn_bias, dropout_p=drop_p, scale=self.scale
            )
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        mask_windows = (
            window_partition(img_mask, self.window_size)
            .permute(0, 2, 3, 1)
            .contiguous()
    ...
    def forward(self, x):
        ...
        if self.input_resolution == x_size:
            attn_windows = self.attn(
                x_windows, mask=self.attn_mask.to(x.dtype)
            )  # (NW*B) x (Ws*Ws) x C
        else:
            attn_windows = self.attn(
                x_windows, mask=self.calculate_mask(x_size).to(x.device, x.dtype)
            )

        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, C)
        shifted_x = window_reverse(
            attn_windows, self.window_size, Ph, Pw
        )  # B x C x Ph x Pw

        # reverse cyclic shift
        if self.shift_size > 0:
            x = torch.roll(
                shifted_x, shifts=(self.shift_size, self.shift_size), dims=(2, 3)
            )
        else:
            x = shifted_x
