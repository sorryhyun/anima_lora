...
class AttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # compute attention via SDPA (flash / mem-efficient backend) so the
        # b,hw,hw map is never materialized — avoids OOM on big f4 latents
        # without xformers. Anima SR sidecar patch (Blackwell, no-xformers).
        b, c, h, w = q.shape
...
class MemoryEfficientAttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, H, W = q.shape
        q, k, v = map(
            lambda x: rearrange(x, "b c h w -> b (h w) c"), (q, k, v)
        )  # b x hw x c

        q, k, v = map(
            lambda t: (
                t.unsqueeze(3)  # b x hw x c x 1
