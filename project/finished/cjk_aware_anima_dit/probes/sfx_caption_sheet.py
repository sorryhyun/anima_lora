...
* **right** — the caption the mirror holds for the page, split by the
  ...
  records through the same ``text_clause`` and a difference is flagged.
    ...
    python project/finished/cjk_aware_anima_dit/probes/sfx_caption_sheet.py            # 87 sincos pages
    python project/finished/cjk_aware_anima_dit/probes/sfx_caption_sheet.py --only mask # pages with a mask-component read
    python project/finished/cjk_aware_anima_dit/probes/sfx_caption_sheet.py --only rule # pages whose kind came from the rule
