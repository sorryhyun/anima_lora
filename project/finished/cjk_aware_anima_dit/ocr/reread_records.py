...
records and reads what the MIT text mask boxed that no detector did.
    ...
    make daemon-run ARGS="--stall-timeout 0 project/finished/cjk_aware_anima_dit/ocr/reread_records.py --stage read"
    # CPU: apply the reads, re-derive kind / reading order, the floor numbers
    python project/finished/cjk_aware_anima_dit/ocr/reread_records.py --stage apply              # --reread all → …_hybrid_vl.jsonl (default)
    python project/finished/cjk_aware_anima_dit/ocr/reread_records.py --stage apply --reread sfx  # arm C11's SFX-only file → …_hybrid_sfx.jsonl
