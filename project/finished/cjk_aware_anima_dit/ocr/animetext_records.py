...
the nested reads YOLO's block-and-columns emit.
    ...
    python project/finished/cjk_aware_anima_dit/ocr/animetext_records.py --stage det apply side hand
    # GPU (daemon), only if the det stage found boxes the O6 probe never read:
    make daemon-run ARGS="--stall-timeout 0 project/finished/cjk_aware_anima_dit/ocr/animetext_records.py --stage read"
