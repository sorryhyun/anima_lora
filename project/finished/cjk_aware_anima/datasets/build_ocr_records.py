...
Two stages, so the GPU pass runs once and the merge is re-runnable on CPU::
    ...
    make daemon-run ARGS="--stall-timeout 0 project/finished/cjk_aware_anima/datasets/build_ocr_records.py --stage spot"
    # CPU: merge, kind, reading order, gate numbers
    python project/finished/cjk_aware_anima/datasets/build_ocr_records.py --stage merge
