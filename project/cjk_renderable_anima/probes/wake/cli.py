"""Command line: flags grouped by the stage *and arm* that reads them.
...
def build_parser(stages, description: str | None = None) -> argparse.ArgumentParser:
    ...
    _data_synth_args(p.add_argument_group("data: S line (plan_synth)"))
    _train_args(p.add_argument_group("train"))
    _train_synth_args(p.add_argument_group("train: S line (plan_synth)"))
    _rows_args(p.add_argument_group("train: rows arm"))
...
def _train_args(g):
    ...
    # (trainables._init_free) and the rows arm uses it as the μ‖f‖² pull on the
