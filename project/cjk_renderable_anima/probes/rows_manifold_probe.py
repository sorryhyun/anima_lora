#!/usr/bin/env python
"""rows_manifold_probe — does a trained ext-row table have structure, and does
...
``--stage adapter`` (Qwen TE + llm_adapter forward; ``--device cuda`` on the
  ...
  frame-to-frame consistency of ``d`` (context-free or not).
    ...
    .venv/bin/python project/cjk_renderable_anima/probes/rows_manifold_probe.py \
        --arm_dir output/wake_probe/rows_synth_full_fm10k_full_s53k_qoff --stage rows
    make daemon-run ARGS="--stall-timeout 0 project/cjk_renderable_anima/probes/rows_manifold_probe.py \
...
# The training frames (stage/scenes.py FRAMES), JA-swapped as the data stage
...
def stage_rows(a):
    ...
    from wake.common import HIRA, KATA
    ...
    try:
        ...
        from wake.render import find_fonts
