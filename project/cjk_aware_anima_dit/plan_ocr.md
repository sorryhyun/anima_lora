...
| readers | `kha-white/manga-ocr-base` (ViT + BERT-ja WordPiece decoder; wrapped as `MangaOCR` in `../cjk_aware_anima/datasets/manga_text.py`, decode-only, no MeCab); PP-OCRv6 ONNX (`anime_tools.ocr._onnx`: `TextDetector` / `TextRecognizer` / `OcrEngine`); VL-1.6 (`models/paddleocr_vl_1.6`, `probes/ocr_vl16_*.py`); **HunyuanOCR-1.5** (`models/hunyuan_ocr`, `eval_*.py --reader hunyuan`) — measured stock on both evals 2026-09-08 and **out**: below stock manga-ocr in-domain, level with stock VL-1.6 on the gate floor, Chinese-first prior (`findings.md` § Outside reader — HunyuanOCR-1.5) | — |
