# CJK-aware Anima, DiT side — history

The dated, per-phase record of this line: every arm, gate and correction in
the order it happened, each with its evidence pointer. Moved out of
[`findings.md`](findings.md) on 2026-09-13 so that file could shrink to the
verdicts alone; nothing here was rewritten. Read
[`findings.md`](findings.md) § Comparability before comparing any two
numbers below — the sincos gate changed units twice and the eval key was
re-based once, and sections are marked where they quote a retired basis.

`reports/…` links resolve under `_archive/cjk_aware_anima_dit/reports/`
(gitignored, private mirror) for everything dated before 2026-09-13 and under
this directory's `reports/` (also gitignored) after. The eight plan files
named below live in `_archive/cjk_aware_anima_dit/plans/`.

---

## The freeze (2026-09-08) and what reopened
...
render line was reframed 2026-09-13 as the wake line ([`cjk_renderable_anima/reports/`](../cjk_renderable_anima/reports/README.md)).
...
on the row → pixel path. Plan: [`cjk_renderable_anima/reports/`](../cjk_renderable_anima/reports/README.md); instrument
...
Training only there (`--t_min 0.7 --t_max 0.9`, same recipe) → **24/36 singles**
(from 10/36), combo CER 0.67, EN 24/24; classifier σ 0.8 top-1 19 → 33/48, the
band sharpened rather than widened. Combos still render one glyph, now often the
first of the string.

Native rendering (scene prompts + clause): the address works only inside the
trained clause grammar and carries its blank-canvas layout prior (scene wiped
on ~half the renders); a Japanese-language clause is dead (its own words hit
competing trained rows). Kanji (`rows_k24_band`, 12 IDS atoms + 12 composites,
same recipe): singles 11/36 both readers vs kana 24/36; atoms render 10/12 on
seed 0, composites decompose into their parts (森 → 木林) and multi-row combos
merge into one character-like glyph (品休 → 和, 相晶加 → 朋) — the DiT composes
kanji at the component level in pixels. Row deltas show only weak additive
structure (4/12 composites ≥ 94th pct vs random sums).
