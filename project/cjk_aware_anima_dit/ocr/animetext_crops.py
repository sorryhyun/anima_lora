#!/usr/bin/env python3
"""Unlabelled text crops from the AnimeText **test** split — the SSL corpus for
``ssl_tower_simmim.py`` (tower domain adaptation, 2026-09-08).

    ANIMA_ANIMETEXT_ROOT=<dir with test-00000-of-00001.parquet> \\
        python project/cjk_aware_anima_dit/ocr/animetext_crops.py [--limit 2000]

``deepghs/AnimeText`` (CC-BY-NC-SA — research build only) is detection-only:
``objects.bbox`` = normalised ``[cx, cy, w, h]``, ``category`` 0 = text, 1 =
hard negative. Only the test parquet (73,725 images / 88,320 boxes, 8.4 GB) is
on disk; text boxes are cut axis-aligned at ``--pad`` 12 % of the box's long
edge per side (the O1 crop convention, no deskew — the polygons live in a
separate file and SSL does not need them), dropped below ``--min_side`` px.

Output, beside the parquet: ``animetext_crops/test/<image_id>_<k>.png`` +
``animetext_crops/manifest_test.parquet`` (``image_id, k, w, h, box, path``,
path relative to the root). No text column — there is none.
"""
...
def animetext_root() -> Path:
    env = os.environ.get("ANIMA_ANIMETEXT_ROOT")
    if not env:
        raise SystemExit(
            "ANIMA_ANIMETEXT_ROOT is not set — the directory holding "
            "test-00000-of-00001.parquet (the AnimeText test split)."
        )
    root = Path(env).expanduser()
    if not (root / "test-00000-of-00001.parquet").is_file():
        raise SystemExit(f"{root} has no test-00000-of-00001.parquet")
    return root
...
def _do_group(args) -> list[dict]:
    parquet, gi, out_dir, pad, min_side, overwrite = args
    ...
    for r in t:
        ...
        for box, cat in zip(obj["bbox"], obj["category"]):
            ...
            if crop is None:
                continue
            fn = out_dir / f"{r['image_id']}_{k}.png"
...
def main():
    ...
    ap.add_argument(
        "--name",
        help="manifest suffix → manifest_test_<name>.parquet (default: smoke "
    ...
    out_dir = root / "animetext_crops" / "test"
    ...
    print(
        ...
        f"row groups; cropping {len(groups)} groups → {out_dir}",
        flush=True,
    )
    t0 = time.time()
    rows: list[dict] = []
    jobs = [
        (str(parquet), gi, out_dir, a.pad, a.min_side, a.overwrite) for gi in groups
    ]
    ...
    mpath = out_dir.parent / f"manifest_test{'_' + name if name else ''}.parquet"
