...
| D5-names | Wikidata lexicon → **name register** | **First slice BUILT 2026-08-17**: `build_pairs.py`'s `names` register (name-only swap in otherwise-EN captions — exact teacher context; see `datasets/README.md`). Still open: composing captions for lexicon names absent from our captions. Risk 4 is now measured in renders — MT transliterates names, so their rows never accumulate visits through D1 alone. |
...
- **Phase 2c — train + eval. FIRST PASS MEASURED 2026-08-16** — packs, G5, the
  ...
  1. **Coverage pass**: D1-wide **DONE** (corpus is built; 45,230 pairs);
     ...
     **(b) name register** (D5) — n1/n2 unchanged, 博:10 麗:31. **First slice
     BUILT 2026-08-17**: the `names` register in `build_pairs.py` swaps *only*
     character/copyright segments to their pinned JA names inside otherwise-EN
     captions (teacher context exact by construction; trained by default,
     `--train_registers tags,tags_alt,names`). The `natural` MT-prose register
     was retired in the same change (span-less ⇒ inert under `loss=span`;
     name-pinning superseded). Still open: composing captions for lexicon
     names that never occur in our captions;
