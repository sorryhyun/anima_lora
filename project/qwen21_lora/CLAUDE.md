...
**This line is not Anima.** The root `CLAUDE.md` describes the Anima DiT, and its
...
schedule. What the two share is `library.runtime.offloading` (the block swapper) and
nothing else; `library/`, `networks/`, `configs/`, `train.py` and the `make` targets are
all Anima-only. This line is plain scripts under `src/`, run through the daemon.
...
| Adapters live in `networks/`, selected by `network_module` | `networks/` is bound to Anima module names. `src/lora.py` keeps its pairs **outside** `transformer_blocks` — the swapper moves every `.weight` under a block, so a nested adapter (peft) pages trainable weights to CPU while their `.grad` stays on the card |
| `make lora`, config chain `base → preset → method → CLI`, `masked_loss` | None of it. `src/{cache_dataset,train_lora,generate,backward_smoke}.py`, flags only |
...
- **Size the swap from a measurement, not a reserve guess.** `train_lora.py::report_fit`
...
- Both encoders are precached (`cache_dataset.py`); training never loads them.
