...
`library/qwen21/CLAUDE.md` first**: the root `CLAUDE.md` invariants (5D latents, max-padded
...
`smoke_t2i`, `bench_accel`, `generate`) on top of `library.qwen21`; caching and training
are `scripts/qwen21/{cache,train}.py`. GPU work goes through the daemon, as everywhere in
this repo.
