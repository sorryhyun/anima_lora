...
Floor arm = `load(seed)` — the **whole** seed table, all 2 274 rows, no
inventory filter ever (`cjk_scale/eval.py::floor_arm`; a filtered floor
renders frozen singles as raw-pack rows, below any honest floor). Job
