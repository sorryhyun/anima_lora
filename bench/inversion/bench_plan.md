...
Destination script: `bench/inversion/tag_slot_analysis.py`
Output root: `bench/inversion/results/tag_slot/`
...
Bundled at `library/anima/configs/qwen3_06b/tokenizer.json`. Load with:

```python
from transformers import AutoTokenizer
tok = AutoTokenizer.from_pretrained("library/anima/configs/qwen3_06b")
enc = tok(caption_text, return_offsets_mapping=True, truncation=True, max_length=512)
```

**Sanity check required:** for each image × variant, assert that `enc["input_ids"]` equals the non-zero prefix of `t5_input_ids_v{vi}` from the cache. If they mismatch, the tokenizer or padding convention has drifted and the position mapping is invalid — fail loudly, don't silently proceed.
...
- **rating**: always tag-index 0 (`explicit`, `sensitive`, `general`, `questionable`, and the curious `absurdres` outlier — include it and let the data decide whether it belongs).
...
**Key class list:**
- `rating=explicit`, `rating=sensitive`, `rating=general` (treat each rating as its own class)
...
For each (caption, tag) pair where the tag appears in multiple variants (which is every tag in every caption, by construction):

1. Collect the 8 slot-vectors from `v0..v7` at the 8 different slot positions the tag lands in.
...
## Phase 5 (follow-up, not part of this bench) — wire into `scripts/invert_embedding.py`
