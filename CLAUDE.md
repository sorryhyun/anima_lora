...
- `img2emb/train_img2emb.py` — Img2emb resampler training dispatcher. Preprocessing (`make preprocess-img2emb` = features + anchor artifacts) is split from training (`make img2emb` = pretrain + finetune) so tuning hyperparams doesn't rerun the heavy one-time steps. SigLIP2 vision tower must be fetched via `make download-siglip2` → `models/siglip2/`. See `scripts/img2emb/README.md` for the anchor spec (pre-injection at query-time + post-injection at output-time, optional teacher forcing via `tf_ratio`).
- `img2emb/test_img2emb.py` — Generate an image from a reference via the img2emb resampler (used by `make test-img2emb`). Without `--image_size`, auto-picks the `CONSTANT_TOKEN_BUCKETS` entry closest to the ref aspect ratio.
