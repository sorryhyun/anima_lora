...
- `img2emb/train.py` — Img2emb resampler training dispatcher (imports each stage's entrypoint and runs them in-process; no subprocesses). Preprocessing (`make preprocess-img2emb` = features + anchor artifacts) is split from training (`make img2emb` = pretrain + finetune) so tuning hyperparams doesn't rerun the heavy one-time steps. TIPSv2-L/14 vision tower must be fetched via `make download-tipsv2` → `models/tipsv2/` (requires `trust_remote_code=True`; aspect-preserving bucketed preprocessing is always on — see `scripts/img2emb/buckets.py`). See `scripts/img2emb/README.md` for the anchor spec (pre-injection at query-time + post-injection at output-time, optional teacher forcing via `tf_ratio`). Stage scripts: `preprocess.py`, `pretrain.py`, `finetune.py`.
