...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=256,
        help="Resampler query count K. Default 256 (hard K-cap; covers ~95%% "
             "of active T5 lengths L). Target is sliced to [:K] and only "
             "slot < min(L, K) is supervised; predictions are zero-padded "
             "back to 512 at eval/inference boundary.",
    )
    ...
    p.add_argument(
        ...
        help="Save an intermediate checkpoint every N steps "
             "(0 = disabled; final ckpt is always saved). "
             "Intermediate files get a `_stepNNNNNN` suffix so the final "
             "filename stays stable for downstream consumers.",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.1,
        help="Weight on multi-positive InfoNCE over pooled per-variant "
             "targets (SupCon-style; positives = all V variants of the same "
             "image). Set 0 to disable.",
    )
    p.add_argument(
        "--infonce_tau",
        type=float,
