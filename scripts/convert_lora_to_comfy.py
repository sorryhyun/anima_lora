#!/usr/bin/env python3
"""Convert LoRA weights between anima_lora and ComfyUI formats."""
...
def main(args):
    ...
    for k in keys:
        if not args.reverse:
            ...
            module_and_weight_name = "_".join(
                k.split("_")[2:]
            )  # Remove `lora_unet_`or `lora_te_` prefix
            ...
            original_module_name = module_name.replace(
                "_", "."
            ...
            original_module_name = original_module_name.replace(
                "adaln.up.self.attn", "adaln_up_self_attn"
            )
            original_module_name = original_module_name.replace(
                "adaln.up.cross.attn", "adaln_up_cross_attn"
            )
            original_module_name = original_module_name.replace(
                "adaln.up.mlp", "adaln_up_mlp"
            )
            original_module_name = original_module_name.replace(
            ...
            if ".lora_" in module_and_weight_name:
                ...
                module_name, weight_name = module_and_weight_name.rsplit(".", 1)
    ...
    if count == 0:
        logger.warning(
            "No keys were converted. Please check if the source file is in the expected format."
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert LoRA format between anima_lora and ComfyUI"
    )
    parser.add_argument(
        "src_path",
        type=str,
        help="source path (anima_lora format, or ComfyUI format if --reverse is set)",
    )
    parser.add_argument(
        "dst_path",
        type=str,
        help="destination path (ComfyUI format, or anima_lora format if --reverse is set)",
    )
    parser.add_argument(
        "--reverse",
        action="store_true",
        help="reverse conversion direction (ComfyUI -> anima_lora)",
    )
