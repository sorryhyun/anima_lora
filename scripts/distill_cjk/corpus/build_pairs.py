...
ids is what says which rows the run can actually move.
    ...
    python -m scripts.distill_cjk.corpus.build_pairs          # CPU
"""
...
def read_commentary_records(path: Path) -> tuple[dict, list[dict]]:
    """Read a D2 ``commentary_pairs.jsonl`` (or the legacy one-line ``{stats, records}`` blob).

    Inlined from the frozen line's ``commentary.py`` so the corpus package does not
    reach back into ``project/``.
    """
...
def add_hant_register(pairs: list[dict], suffix: str = "_zh") -> list[dict]:
    ...
    """
    from scripts.distill_cjk.corpus import tag_glossary  # noqa: PLC0415  (sibling — OpenCC helpers)
...
def build_d2(args, rng) -> list[dict]:
    ...
    if not args.commentary.exists():
        ...
        return []
    _, records = read_commentary_records(args.commentary)
