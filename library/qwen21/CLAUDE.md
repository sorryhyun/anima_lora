...
| Block-compile FIRST on OOM, not grad checkpointing | **Activation checkpointing is the lever** — 32 blocks at ~4096 tokens is what does not fit, and compile is a speed lever, not a memory one, **on by default** (`torch.compile(dynamic=True)`): ±0 at 512² with 12 swaps (PCIe-bound), −11 % at 1024² with 7 swaps (compute-bound; swap 14→5 changed nothing). `--compile_seq bounded` = automatic dynamic + `mark_dynamic` over the cache's joint-token range, same speed |
