            ...
            if use_cached_info_for_subset:
                ...
                if strategy is not None:
                    ...
                    for i, img_path in enumerate(tqdm(img_paths)):
                        l = len(os.path.splitext(img_path)[0])
                        found = False
...
class FineTuningDataset(BaseDataset):
    def __init__(
        ...
        for subset in subsets:
            ...
            npz_paths = sorted(npz_paths, key=lambda item: len(os.path.basename(item)), reverse=True)
            ...
            for image_key in image_keys_sorted_by_length_desc:
                ...
                if image_size is not None:
                    image_info.image_size = tuple(image_size)
...
class ControlNetDataset(BaseDataset):
    def __init__(
        ...
        for image_key, info in self.dreambooth_dataset_delegate.image_data.items():
            ...
            ctrl_img_path = os.path.abspath(ctrl_img_path)

            info.cond_img_path = ctrl_img_path
            cond_imgs_with_pair.add(os.path.splitext(ctrl_img_path)[0])

        extra_imgs = []
        for subset in subsets:
            conditioning_img_paths = glob_images(subset.conditioning_data_dir, "*")
            conditioning_img_paths = [os.path.abspath(p) for p in conditioning_img_paths]
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "cudagraphs", "ofi", "fx2trt", "onnxrt", "tensort", "ipex", "tvm",
        ],
    ...
    parser.add_argument(
        "--lowram",
        action="store_true",
        help="enable low RAM optimization.",
    )
    parser.add_argument(
        "--highvram",
        action="store_true",
        help="disable low VRAM optimization.",
    )
...
def add_masked_loss_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--masked_loss",
        action="store_true",
        help="apply mask for calculating loss.",
    )
...
def verify_command_line_training_args(args: argparse.Namespace):
    ...
    if getattr(args, "config_file", None) is not None:
        logger.info(
            f"wandb is enabled, but option `config_file` is included in the command line. Please be careful about the information included in the path."
        )

    if args.huggingface_repo_id is not None and args.huggingface_repo_visibility != "public":
        logger.info(
            f"wandb is enabled, but option huggingface_repo_id is included in the command line and huggingface_repo_visibility is not 'public'. It is recommended to move it to the `.toml` file."
        )
...
def add_sd_saving_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--use_safetensors",
        action="store_true",
        help="use safetensors format to save",
    )
    ...
    if optimizer_type == "Lion".lower():
        ...
        if optimizer_type == "AdamW8bit".lower():
            ...
            if "momentum" not in optimizer_kwargs:
                logger.warning(f"8-bit SGD with Nesterov must be with momentum, set momentum to 0.9")
        ...
        if "momentum" not in optimizer_kwargs:
            logger.info(f"SGD with Nesterov must be with momentum, set momentum to 0.9")
            optimizer_kwargs["momentum"] = 0.9
        ...
        if not optimizer_kwargs["relative_step"] and optimizer_kwargs.get("warmup_init", False):
            logger.info(f"set relative_step to True because warmup_init is True")
            optimizer_kwargs["relative_step"] = True
        ...
        if optimizer_kwargs["relative_step"]:
            ...
            if args.max_grad_norm != 0.0:
                logger.warning(f"because max_grad_norm is set, clip_grad_norm is enabled. consider set to 0")
