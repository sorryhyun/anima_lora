...
They share the same model family lineage (`MiniTrainDIT`) and load the same base checkpoints for the transformer blocks, but the two forward paths differ in several visible ways. Check this list when a workflow behaves differently between `inference.py` and ComfyUI — the transformer-block math itself is identical.

It matters most for anything that hooks the forward path — notably `mod_guidance.py` in [ComfyUI-Spectrum-KSampler](https://github.com/sorryhyun/ComfyUI-Spectrum-KSampler) and the per-block mod-guidance scheduling documented in [`docs/inference/mod-guidance.md`](../inference/mod-guidance.md).
...
anima_lora — `library/anima/models.py::Anima.__init__` (`self.pooled_text_proj` assignment):
...
Zero-initialized output layer (`library/anima/models.py::Anima.init_weights`) so it's a no-op at init, trained via `project/finished/mod_guidance/distill.py`.

Used in `forward_mini_train_dit` at `library/anima/models.py::Anima.forward_mini_train_dit`:
...
anima_lora — `library/anima/models.py::Anima._preprocess_text_embeds` computes `crossattn_seqlens` from the target attention mask, returns `(context, seqlens)`. The seqlens tensor is threaded all the way into the flex-attention block mask inside `forward_mini_train_dit` (`_crossattn_mask_mod`):
...
A `flash4` branch stub remains in `attention_dispatch.py` but is disabled (`docs/optimizations/fa4.md`).
...
Practical consequence. ComfyUI runs Anima eagerly with per-bucket shape changes, which is fine because it also doesn't use `torch.compile`. Running anima_lora's compiled path in ComfyUI would need both `compile_blocks` wiring and compile-compatible latent resolutions; it is not supported.

### 2.3 Block forward signature

anima_lora (`library/anima/models.py::Block._forward`):
...
anima_lora (`library/anima/models.py::Anima.forward_mini_train_dit`, the `self.final_layer(...)` call):
...
|---|---|---|
| `torch.compile(block._forward)` | `compile_blocks()` at `library/anima/models.py::Anima.compile_blocks` | none |
| `@torch._disable_dynamo` on unsloth checkpoint wrapper | yes, `library/anima/models.py::unsloth_checkpoint` | N/A |
...
anima_lora manages VRAM and compilation inside the model (16 GB training and inference, compile across bucket shapes); ComfyUI leaves VRAM to `model_management` and runs the forward eagerly.
...
pooled-text extraction (`max(dim=1).values` over `crossattn_emb`) happens in anima_lora inside `library/anima/models.py::Anima.forward_mini_train_dit` as part of the `pooled_text_proj` flow. In ComfyUI it happens only inside the custom mod-guidance node (`mod_guidance.py`), because the vanilla forward has no reason to pool.
