...
Method TOMLs live in `configs/methods/`; the per-variant files in
`configs/gui-methods/` run with `make lora-gui GUI_PRESETS=<variant>`.
...
of autocast; stored parameters stay bf16. `--lora_fp32_accumulation` is ignored.

## Caption shuffle variants

Pre-cached caption permutations, one drawn per batch item per epoch.
